...
Each module is a bit in a 64-bit `ModuleManifest` field (uint64 LE) in the Superblock:
...
| 3 | Tags (TAGS) | TagIndexRegion | +184 |
...
Inode sizes: 512B (standard) or 1024B (hyperscale, when module bytes exceed 512B). The inode core is 312B (identity + timestamps + extents + ActiveModules + overflow). Remaining bytes are claimed by active modules. Padding bytes are reserved for future module additions without migration.
...
|---------|---------|-----------|----------|
| Minimal | Core only | 512B | ~1.6% |
| Standard | SEC + INTG + SNAP + CMPR | 512B | ~1.6% |
| Enterprise | SEC + CMPL + INTL + TAGS + INTG + SNAP + CMPR + ALOG | 1024B | ~3.2% |
| Maximum Security | ALL 19 modules | 1024B | ~3.2% |
| Edge/IoT | STRM + INTG (512B blocks) | 512B | ~1.6% |
| Analytics | INTL + CMPR + SNAP + QURY | 512B | ~1.6% |
| Fully Custom | User decides at deployment | 512B or 1024B | ~1.6% to ~3.2% |
...
3. **Emergency Recovery Block**: Fixed at block 14 (always same location). Plaintext VDE UUID, creation date, admin contact. Accessible even with all keys lost.
...
│  Inode layout: Standard512 (fits most workloads)                │
...
14. **Dynamic Scaling Architecture** -- All structural limits are configurable, not constant. The principle: DW scales unlimited, restricted only by hardware.
    ...
    - **Region Directory**: 127 slots per block (fits in 1 block). Slot 126 is optionally an overflow pointer (RDIR_OVERFLOW) giving 126 usable region slots per block. If 126 regions are insufficient (unlikely — 19 modules use ~25 regions), the overflow pointer chains to additional blocks extending capacity to 250+ slots.
...
19. **VDE Scalable Internals — Static Format, Adaptive Subsystems** -- The on-disk VDE format is STATIC (superblock, region directory, magic bytes never morph). Each subsystem INSIDE the VDE uses adaptive strategies that auto-tune based on scale. This is the AIE philosophy applied to everything else.
    ...
    - **Inode: Variable-Width with Inline Data:**
      ...
      - **Standard inode (512 bytes):** v2.1 composable format with 6×32B extent pointers, ActiveModules bitmap, inline tags, and module fields. Supports files up to 64GB (4KB blocks) via indirect/double-indirect extent blocks.
      ...
      - **Inode layout:** Superblock declares `InodeSize: 512 | 1024` via InodeLayoutDescriptor. 512B is standard; 1024B (hyperscale) is used when active modules exceed 512B of total inode data. Configurable at VDE creation.
