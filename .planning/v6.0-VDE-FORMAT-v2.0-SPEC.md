...
│  Allocation size:   uint24 (bits 40-63)  → in 64B units     │
│          max = 16,777,215 × 64 = 1.07 GB per inode          │
└─────────────────────────────────────────────────────────────┘
```
...
public struct IsdEntry
{
    ...
    public uint Size => (uint)(_data >> 40);            // Top 24 bits
}
```
...
**ISD Memory Model:**
The ISD MUST be accessed via memory-mapped I/O (`System.IO.MemoryMappedFiles.MemoryMappedFile`), never heap-allocated. The OS virtual memory subsystem page-faults only the 4KB pages containing active ISD entries into physical RAM. An 800MB ISD for 100M inodes consumes near-zero physical memory when sparse. This avoids CLR Large Object Heap (LOH) allocation, GC pauses, and memory fragmentation.
...
```
The 8-byte ISD entry is ≤ one disk sector (512B), so Step 3 is atomic on all modern drives. Crash scenarios:
...
void DeleteInode(ulong inodeNum)
{
    ...
    ulong isdAddr = SMA_BASE + 4096 + (inodeNum * 8);      // pure math — O(1)
    var entry = ReadIsdEntry(isdAddr);
    if (entry.Size > 0)
    {
        FreeSmaBlocks(entry.Offset, entry.Size);            // free SMA allocation
        WriteIsdEntry(isdAddr, default);                    // zero the ISD entry
    }
}
```
...
**Decision:** All SMA writes follow strict payload-first, fsync-barrier, then ISD-entry-update ordering. The ISD entry is the last thing written, and at 8 bytes it fits within a single 512B disk sector — making Step 3 atomic on all modern storage hardware (NVMe, SATA, SAS). Crash recovery is passive: the Background Scrubber cross-references the ISD against the inode bitmap and reclaims any orphaned SMA blocks. No explicit crash log or redo journal is required for the SMA write path.

**Rationale:** This is the same "data before metadata" crash-ordering principle used by ext4 `data=ordered` mode and ZFS copy-on-write. It guarantees that a crash can leave orphaned-but-harmless blocks (scrubber reclaims them) but can never leave a valid ISD entry pointing to garbage data. The 8-byte atomic write eliminates the need for a two-phase commit or SMA-specific WAL entry.
