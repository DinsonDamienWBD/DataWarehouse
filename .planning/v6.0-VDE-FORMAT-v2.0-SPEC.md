...
A full 32-byte BLAKE3 hash of the concatenated executable payloads (kernel || SPDK || runtime) is stored at the byte offset specified by `ContentHashOffset` in the preamble header. This provides cryptographic integrity verification of all executable content beyond the 8-byte truncated header checksum. The hash covers ONLY the executable payloads, EXCLUDING alignment padding bytes between the runtime end and VdeOffset.
...
**Addressing width constraint:** KernelOffset, KernelSize, SpdkOffset, SpdkSize, RuntimeOffset, and RuntimeSize are uint32, limiting each component to < 4 GiB and the total preamble to < 4 GiB. PreambleTotalSize and VdeOffset are uint64 for future-proofing but MUST NOT exceed 2^32 in v2.1 (enforced by the uint32 component offsets). If a future version requires preambles > 4 GiB (e.g., bundled AI models), the header version must be bumped and the uint32 fields promoted to uint64 with a new 96-byte header layout.

`VdeOffset` MUST be the first 4 KiB-aligned offset >= `PreambleTotalSize`. If `PreambleTotalSize` is already 4 KiB-aligned, `VdeOffset == PreambleTotalSize`. Otherwise, `VdeOffset = ceil(PreambleTotalSize / 4096) * 4096`. Padding bytes between `PreambleTotalSize` and `VdeOffset` MUST be zero-filled. Block 0 of the VDE starts at `VdeOffset`.
...
if bytes[0..7] == "DWVD-BOO":   // 8-byte truncation of "DWVD-BOOT" — preamble present
    ...
    // Block 0 starts at offset 0. Distinguish v2.1 from legacy v1.x by reading FormatMajor.
    major = file.ReadByte(4)   // FormatMajor at Superblock offset 0x04
    if major >= 2:
        vde_block0 = file.Seek(0)   // v2.1+ preamble-free VDE
        // INCOMPAT_HAS_PREAMBLE (bit 8) MUST be clear in IncompatibleFeatureFlags
    else:
        vde_block0 = file.Seek(0)   // Legacy v1.x (read-only compatibility mode)
else:
    // No DWVD magic at byte 0 and no DWVD-BOO preamble.
    // This is NOT a DWVD file — reject immediately.
    throw InvalidVdeFormatException("Unrecognized file format")
```

Tools and file identification utilities SHOULD check for `DWVD-BOO` (bytes 0-7) first (preamble-present VDE), then check for `DWVD` at bytes 0-3 and distinguish v2.1+ from v1.x via `FormatMajor` at offset 4. A preamble-free v2.1 VDE always has `DWVD` at byte 0 (Superblock Magic) followed by `FormatMajor >= 2` at byte 4. Legacy v1.x files have `FormatMajor < 2` and are opened in read-only compatibility mode.
...
**Preamble crash recovery:** If the system crashes mid-preamble-update, the BLAKE3 content hash will fail verification on next boot. In this case, the VDE engine skips the preamble and operates in hosted mode only (no bare-metal boot). The preamble can be re-written via `dw preamble update --force`. VDE data is never affected by preamble corruption.
...
| Block Size | Metadata TRLR Tax (16/BlockSize) | Data Region Tax (separated) | Use Case |
|-----------|---------|---------|----------|
| 4KB (default, minimum) | 0.39% | 0.39% (1 TRLR per 255 data = 1/256) | General purpose |
| 16KB | 0.098% | 0.39% (1 TRLR per 255 data) | Streaming, large objects |
| 64KB | 0.024% | 0.39% (1 TRLR per 255 data) | Video, scientific data |

**Minimum block size:** 4096 bytes. Sub-4K block sizes are not supported — the Superblock, inode layout, and all byte-offset calculations assume BlockSize ≥ 4096.
...
-8      8     XxHash64          (ulong: checksum of bytes [0..BlockSize-17], i.e., all bytes before the 16B trailer)
```

**Computation Order (Superblock Block 0):** When writing Superblock Block 0, the field write order MUST be: (1) Compute HeaderIntegritySeal = HMAC-BLAKE3 over bytes [0x0000..0x0FCF], write to offset 0x0FD0. (2) Compute XxHash64 over bytes [0x0000..0x0FEF] (includes the seal), write to offset 0x0FF8. (3) Write GenerationNumber to offset 0x0FF4. Step 2 depends on step 1 because the XxHash64 range includes the 32-byte HMAC seal. Reversing this order would create a circular dependency.
...
**Security Trade-off:** XxHash64 is a non-cryptographic hash chosen for hot-path performance (~30 GB/s on x86-64 with SSE2; ~10-15 GB/s on ARM64 with NEON; multi-GB/s on all target architectures). It is NOT collision-resistant. A malicious actor with raw block device access could forge metadata blocks with valid XxHash64 checksums. Defense-in-depth layers mitigate this:
...
Block Type Tags: SUPB (Superblock), RMAP (Region Map), POLV (Policy Vault), ENCR (Encryption Header), BMAP (Bitmap), INOD (Inode), TAGI (Tag Index), MWAL (Metadata WAL), MTRK (Merkle Tree), BTRE (B-Tree), SNAP (Snapshot), REPL (Replication State), RAID (RAID Metadata), INTE (Intelligence Cache), STRE (Streaming Append), XREF (Cross-VDE Reference), WORM (Write-Once Region), CODE (Compute Code Cache), DWAL (Data WAL), DATA (Data Block), TRLR (Trailer), FREE (Free/Unallocated), RSTA (Recovery State — used by online recovery operations to mark blocks under active recovery), OPJR (Online Operations Journal), CMVT (Compliance Vault), ALOG (Audit Log), CLOG (Consensus Log), DICT (Dictionary), MLOG (Metrics Log), WALS (WAL Subscriber Cursors), ZNSM (ZNS Zone Map), RCVR (Recovery Footer), ANON (Anonymization Table)
...
**Trade-off:** Individual data blocks cannot self-verify without their corresponding trailer block. Recovery tools locate trailer blocks deterministically using the logical data block index (zero-based, counting only data blocks, not TRLR blocks):

```
trlr_physical_offset = floor(logicalDataBlockIndex / 255) * 256 + 255
data_physical_offset = logicalDataBlockIndex + floor(logicalDataBlockIndex / 255)
```

Both offsets are relative to the start of the Data Region. Example: logical data block 254 → physical 254, TRLR at physical 255. Logical data block 255 → physical 256, TRLR at physical 511.
...
### Block 0 — Primary Superblock (4048 usable bytes: 4096 - 32B HMAC seal - 16B block trailer)
...
127 region pointer slots × 32 bytes per slot = 4064 bytes, plus 16 bytes of zero-fill padding, plus 16 bytes Universal Block Trailer = 4096 bytes total. Each slot: `RegionTypeId(4), Flags(2), ShardId(2), StartBlock(8), BlockCount(8), UsedBlocks(8)` — 32 bytes.
...
320     176    InlineTagArea            Up to 5 compact tags (176B ÷ 32B/tag = 5 full slots + 16B):
                                        ...
                                        (176B = 5 complete 32B tags + 16B. The 16B remainder
                                        CAN hold a 6th compact tag if its total size ≤ 16B
                                        (10B header + ≤6B value). Otherwise it is zero-fill padding.
...
`COMPLEX_FLAGS_MASK` covers bits 3-5, 8-15, 16, and 18 (flags requiring algorithmic work: Shared_COW copy-on-write resolution, Delta_Extent, Torn_Write_Boundary, RAID, crypto offload, HOLE sparse-extent handling, TOMBSTONE deleted-data proofs, etc.). Bits 0-2 and 6-7 (compression class, QoS) are simple O(1) table lookups. Bits 17 (PRECOMPRESSED) and 19-22 (DeadlineTier) are Tier 2 variant-path lookups. Note: the 3-tier XOR dispatch applies to the **READ** path. WRITE operations have a separate path that handles Shared_COW allocation and reference counting.
...
496     16     Inode Slot Integrity Trailer [INOD][Gen][XxHash64]
                                    (Per-slot integrity within the 4KB inode page. This is NOT
                                    the Universal Block Trailer of the 4KB physical block — that
                                    lives at offset 4080 of the 4KB block and covers all 8 inode
                                    slots within that physical page.)
```
...
- Total capacity: 16B + 480B = 496B (single overflow, no chaining needed), 16B + 472B (first overflow, has next pointer) + 480B (second overflow, terminal) = 968B (two overflows)

Maximum module payload for ALL 39 modules simultaneously: ~1236B (269B inline inode module fields + 16B DataOsModuleArea + ~951B overflow modules). Two chained overflow blocks (968B) handles the v6.0 subset (~448B); three blocks (1440B available) are needed for the full v7.0 theoretical maximum (~1236B).
...
The overflow block has its own 4-byte CRC32C at offset 12, covering bytes [16..495] (the PackedModules area). The InodeNumber (bytes [0..7]), Type/Flags/ModuleCount (bytes [8..11]), and the CRC32C field itself (bytes [12..15]) are NOT covered by this CRC. The InodeNumber is verified by matching against the parent inode's `ModuleOverflowBlockPtr` target. The ModuleCount field is validated structurally during parse (sum of module sizes must equal payload consumed). When `HAS_NEXT_OVERFLOW` flag is set in byte 9 (Flags), the last 8 bytes of the PackedModules area (bytes [488..495]) contain the next-overflow block pointer, reducing usable module payload from 480B to 472B. The primary inode's ExpectedHash/ContentHash (in the Integrity module) covers ONLY the primary 512B block. This localizes integrity verification — checking base inode properties never requires reading the overflow block.
...
| Theoretical maximum (v7.0) | ALL 39 modules | ~1236B | Yes (3 chained overflow blocks, 1440B capacity) |
...
| 1 TB | 268,435,456 | ~1.4% | ~2.9% | 2.9% | 995 GiB |
| 1 PB | 274,877,906,944 | ~1.4% | ~2.9% | 2.9% | 1018 TiB |

Overhead doubles from ~1.4% to ~2.9% at scale (default profile, Integrity Tree at Level 0). Largest contributors: Data WAL (1%), Streaming Append (1%, configurable). Both configurable — setting streaming to 0% reduces to ~1.9%. Comparable to ZFS (3-5%) and Btrfs (3-4%). With Integrity Tree at Level 2+, total rises to ~3.7%.
...
| Integrity Tree (Merkle, Level 2+) | 8.1 GiB | 0.79% |
| Streaming Append | 10.2 GiB | 1.0% |
| Data WAL | 10.2 GiB | 1.0% |
| Block Trailers (separated TRLR) | 4.0 GiB | 0.39% |
| Other (snapshot, repl, RAID, etc.) | ~35 MiB | ~0.003% |
| **TOTAL (default profile)** | **~29.5 GiB** | **~2.9%** |

Note: The default profile excludes Integrity Tree (Level 0 = no Merkle tree). With Integrity Tree at Level 2+, total rises to ~37.6 GiB (~3.7%). The listed individual sizes sum to ~37.6 GiB; the default total excludes the 8.1 GiB Integrity Tree. Both Data WAL and Streaming Append are configurable (setting streaming to 0% reduces default total to ~19.3 GiB / ~1.9%).
...
4. **Inline tag storage** — converts 2+ I/O to 1 I/O for ~85% of tagged objects
...
39. **Always-Bootable Preamble — v6.0 Production Design** — Supersedes the v7.0 Quine VDE deferral. The original four objections are resolved by relocating the bootable content to a pre-format preamble region rather than Block 0: (1) magic conflict resolved — `DWVD-BOO` at byte 0, `DWVD` magic at `vde_offset`; (2) space constraint resolved — preamble is outside the 4048B Superblock field envelope; (3) UEFI FAT32 constraint resolved — firmware reads from the preamble via standard boot sector convention; (4) bootkit surface resolved — preamble is pre-format, read-only at mount, and BLAKE3-checksummed before execution. **Superseded by AD-40:** the preamble is OPTIONAL, not unconditional. When enabled, the ~35-50 MB overhead is outside the VDE capacity model. Composition controls which kernel drivers, SPDK transport, and plugin subset are bundled.
...
**ZNS Hardware Constraint:** ZNS zones forbid in-place overwrites — all writes must be sequential within a zone, and zones can only be reset as a whole. DWVD's fixed-offset metadata regions (Superblock, Bitmap, Inode Table) require random read-modify-write access. Therefore, ZNS deployment MUST use a dual-namespace topology: metadata regions reside on a conventional NVMe namespace (or the SSD's conventional zone capacity), while only the Data Region (`DATA` and `TRLR` blocks) maps to ZNS zones. The VDE engine auto-detects this at mount time via NVMe Identify Namespace and routes metadata I/O to the conventional namespace. No format change is required — the Region Directory's block pointers are namespace-relative, and the engine manages the namespace routing transparently.
...
When `SPATIOTEMPORAL` (InodeFlags bit 5) is set, the inode's 6 × 32B extent slots are reinterpreted as 3 × 64B spatiotemporal extents. `ExtentCount` holds the count of 64-byte spatiotemporal extents (max 3 direct). Indirect blocks hold `floor(4080 / 64) = 63` spatiotemporal extents per block. Depth capacities: depth 1 (≤3 direct), depth 2 (≤3 + 63 = 66), depth 3 (≤3 + 63 × 63 = 3,972). Format:
```
...
- On VDE grow: (1) write new footer at new EOF, (2) mark old footer block as RESERVED in bitmap (not allocatable), (3) update Superblock with new size, (4) only then free old footer block in bitmap. If crash during move, old footer remains valid (volume just doesn't know about newly added space — safe). The old footer block MUST remain RESERVED until step 4 to prevent data writes from overwriting it before the new footer is confirmed.
...
**Footer identification:** The footer uses the same `DWVD` magic signature and `SUPB` block type tag. Recovery tools scan backward from EOF in 4096B steps looking for `SUPB` trailer + valid HMAC-BLAKE3 seal. If no valid footer is found within 1 MB backward from EOF, the scan MUST abort and report no recovery footer found (prevents false positive matches against random data blocks). For volumes that have been grown, the footer may not be at the final EOF position; the `ExpectedFileSize` in a successfully read Superblock provides the authoritative footer location.

**Sparse file optimization (MANDATORY for thin-provisioned VDEs):** When `ThinProvisioningEnabled` (Superblock +0xA2) is set, the VDE file may contain large sparse holes. Recovery tools MUST NOT step backward byte-by-byte through sparse regions — this would issue billions of zero-returning reads. Instead, recovery tools MUST use platform-native sparse extent APIs to skip holes: `FIEMAP` / `SEEK_DATA`/`SEEK_HOLE` on Linux, `FSCTL_QUERY_ALLOCATED_RANGES` on Windows. The backward scan jumps directly to the end of each allocated region, testing only physically-backed blocks for `SUPB` trailers.
...
### Domain 3: 512-Byte Composable Inode — Up to ~1236 Bytes
With ALL 39 modules active, total module payload reaches ~1236B (269B inline + 16B DataOsModuleArea + ~951B overflow). Primary area holds 16B DataOsModuleArea; overflow block(s) hold the rest. Realistically, a file uses 5-7 heavy modules (~350B), fitting in a single overflow block. The v6.0 subset (excluding v7.0 reserves) totals ~448B.
...
The first 16 bytes of Superblock Block 0 form the magic signature. In preamble-free VDEs, these are at file offset 0x00. In preamble-present VDEs, these are at file offset `VdeOffset`. The VDE Engine Detection Logic (above) determines the Block 0 location before reading this signature.
...
Located within Superblock Block 2 (offset `2 × BlockSize` from file start; 0x2000 at default 4 KiB block size), the Namespace Registration Block embeds the VDE's identity within the `dw://` namespace system.
...
ExpectedFileSize == PreambleTotalSize + TotalAllocatedBlks * BlockSize + BlockSize

// When preamble is absent (INCOMPAT_HAS_PREAMBLE bit 8 clear):
ExpectedFileSize == TotalAllocatedBlks * BlockSize + BlockSize

// The trailing "+ BlockSize" accounts for the Recovery Footer (one block at EOF).
// TotalAllocatedBlks does NOT include the Recovery Footer block.
```
...
1    Compliance         CMPL    CMVT (0x434D5654)             ComplianceVault +                12
                                (uses ALOG region, owned by   AuditLog (shared with bit 18)     (PassportSlot:4, ClassLabel:4,
                                 bit 18 ALOG module)                                             SovZone:2, RetentionId:2)
                                DEPENDENCY: CMPL config level >= 3 requires ALOG (bit 18)
                                active in ModuleManifest. Engine MUST auto-activate ALOG
                                if CMPL level 3+ is set and ALOG is inactive.
...
| `0x0000FFFF` | `0000 1111 1111 1111 1111` | All 16 core modules (bits 0-15; not all have inode fields) |
...
Note: Binary representations above are truncated to significant bits for readability. ModuleManifest is always uint64 LE (64 bits).
...
0x01C0  8B    InnerVdeInodeNumber            uint64 LE (0 if top-level; matches inode number width)

──── RESERVED (3592B) ────────────────────────────────────────────────────────────────
0x01C8  3592B Reserved                       Zero (future expansion, to 0x0FCF)
...
0x0FF4  4B    GenerationNumber               uint32 LE (monotonic within epoch; see epoch crash note)
0x0FF8  8B    XxHash64                       uint64 LE over [0x0000..0x0FEF]
                                             Note: XxHash64 covers [0x0000..0x0FEF] (4080 bytes), which
                                             intentionally EXCLUDES the 16B trailer itself (BlockTypeTag +
                                             GenerationNumber + XxHash64). The GenerationNumber is written
                                             last in the atomic write sequence and cannot be included in
                                             the hash that precedes it. BlockTypeTag is protected by the
                                             HMAC seal (Superblock) or by the WAL entry type (metadata blocks).
                                             For data blocks, the separated TRLR record's checksum covers
                                             the full 4096B data block — no trailer exclusion applies.
                                                                   Total: 4096 bytes
```

**Verification:** 16 + 48 + 32 + 16 + 48 + 24 + 40 + 32 + 20 + 28 + 64 + 16 + 8 + 36 + 28 + 3592 + 32 + 16 = 4096 bytes exact. Defined section fields = 456B (includes 8B InnerVdeInodeNumber), HMAC seal = 32B, TRLR = 16B, Reserved padding = 3592B, total = **4096 bytes exact** ✓
...
Example 2: SEC + TAGS + REPL + INTL
  Core:             312 bytes
  + SEC:             24 bytes → 336
  + TAGS:           184 bytes → 520  (InlineTagCount:4 + TagOverflowBlock:4 + InlineTagArea:176)
  + REPL:             8 bytes → 528
  + INTL:            12 bytes → 540
  Raw:              540 bytes
...
Example 4: All 39 registered modules (bits 0-38, maximum envelope)
  ...
  + CMPL:            12 → 348      (bit 1)
  + INTL:            12 → 360      (bit 2)
  + TAGS:           184 → 544      (bit 3: InlineTagCount:4 + TagOverflowBlock:4 + InlineTagArea:176)
  + REPL:             8 → 552      (bit 4)
  + RAID:             4 → 556      (bit 5)
  + STRM:             8 → 564      (bit 6)
  + COMP:             0 → 564      (bit 7, region-only)
  + FABR:             0 → 564      (bit 8, region-only)
  + CNSS:             0 → 564      (bit 9, region-only)
  + CMPR:             4 → 568      (bit 10)
  + INTG:             0 → 568      (bit 11, region-only)
  + SNAP:             0 → 568      (bit 12, region-only)
  + QURY:             4 → 572      (bit 13)
  + PRIV:             2 → 574      (bit 14)
  + SUST:             4 → 578      (bit 15)
  + TRNS:             1 → 579      (bit 16)
  + OBSV:             0 → 579      (bit 17, region-only)
  + ALOG:             0 → 579      (bit 18, region-only)
  + CPSH:             0 → 579      (bit 19, overflow)
  + EKEY:             0 → 579      (bit 20, overflow)
  + WALS:             0 → 579      (bit 21, region-only)
  + DELT:             0 → 579      (bit 22, DataOsModuleArea fixed-offset)
  + ZNSM:             0 → 579      (bit 23, region-only)
  + STEX:             0 → 579      (bit 24, DataOsModuleArea fixed-offset)
  + SDUP:             0 → 579      (bit 25, overflow, v7.0 reserve)
  + ZKPA:             0 → 579      (bit 26, overflow, v7.0 reserve)
  + SPOL:             0 → 579      (bit 27, DataOsModuleArea fixed-offset)
  + OPJR:             0 → 579      (bit 28, region-only)
  + DREC:             0 → 579      (bit 29, superblock-only)
  + TOMB:             0 → 579      (bit 30, extent-flag-only)
  + SWLV:             2 → 581      (bit 31)
  + QSIG:             0 → 581      (bit 32, overflow)
  + STEG:             0 → 581      (bit 33, overflow, v7.0 reserve)
  + QOS:              0 → 581      (bit 34, Module Overflow Block)
  + VECQ:             0 → 581      (bit 35, overflow)
  + ANCR:             0 → 581      (bit 36, overflow)
  + MACR:             0 → 581      (bit 37, overflow)
  + WLCK:             0 → 581      (bit 38, overflow)
  Raw:              581 bytes (269 inline module bytes + 312 core)
  Fixed tier:      1024 bytes (hyperscale — 581 > 512, promoted; requires HYPERSCALE_INODES flag)
  Padding:          443 bytes
  InodeSize = 1024
...
Tags                 InlineTagArea (176B) in inode,              Tags stored in external inverted     Tags as key-value pairs
                     ...
                     Single I/O for ~85% of tagged objects.       1-2 extra I/O per tag query.         properties. Full inode
...
ModuleManifest:   0x0000007DF9FFFFFF (all 36 v6.0-implementable modules active)
                  Binary: bits 0-24, 27-32, 34-38 set
                  Excluded: SDUP (bit 25), ZKPA (bit 26), STEG (bit 33) — v7.0 reserve
Modules:          All 36 v6.0 modules enabled (3 v7.0-reserve modules excluded)
InodeSize:        1024 bytes (312 + 269 module fields = 581 raw → hyperscale, 443 padding)
ModuleConfig:     All active modules at maximum level (0xF); v7.0 reserves at 0x0
ModuleConfig val: 0xFFFFFFFFFFFFFFFF (modules 0-15 at 0xF)
ModuleConfigExt:  0xFFFFF00FFFFFFFFF (modules 16-24 at 0xF, 25-26 at 0x0, 27-31 at 0xF)
ModuleConfigExt2: 0x000000000FFFFF0F (module 32 at 0xF, 33 at 0x0, 34-38 at 0xF, 39-47 at 0x0)
...
ModuleManifest:   User-selected (any combination of v6.0-implementable module bits 0-38,
                  excluding v7.0 reserves: bits 25, 26, 33)
...
**Chain atomicity (crash safety):** Each new ALOG entry (including its EntryHash) is first written to the Metadata WAL as a single atomic WAL record before being appended to the ALOG region. The region header's ChainHash is updated only after the WAL record is committed and the entry is durably written. On crash, WAL replay either completes the append (writing both the entry and the updated ChainHash) or discards the incomplete entry entirely. The chain is never left in a half-written state.

**External trust anchor:** The ALOG chain is self-referential; an attacker with full storage control could rewrite the entire chain consistently. To mitigate this, the VDE engine periodically exports the current ChainHash (from the ALOG region header at offset +0x38) to the Superblock Block 3 `MetadataChainHash` field, creating an external anchor outside the ALOG region. For maximum tamper resistance, deployments SHOULD additionally export the chain head hash to an external trust anchor (e.g., blockchain notarization service, HSM-sealed log, or remote append-only ledger) at a configurable interval (default: every 1000 entries or every 1 hour, whichever comes first). The external anchor interval is stored in the Policy Vault as a type tag 0x0005 record.
...
**Integrity hash rationale:** CLOG entries use CRC-32C (not BLAKE3) because the consensus log's integrity is guaranteed by the consensus protocol itself (quorum agreement across nodes). CRC-32C detects accidental corruption (bit-flips, torn writes) at 4 bytes per entry vs. 32 bytes for BLAKE3. Tamper detection is not the CLOG's responsibility -- an attacker who modifies a local CLOG entry is detected when it fails to match the quorum-committed log on other nodes. For VDEs requiring tamper-evident consensus logs (e.g., regulatory audit), enable the ALOG module alongside CLOG: every committed CLOG entry is then cross-referenced in the tamper-evident ALOG hash chain.
...
**Dictionary directory (immediately after header, 256 entries x 29 bytes = 7424 bytes):**
...
**Accepted trade-off -- security posture leakage:** The plaintext `ModuleManifest` field reveals which VDE modules were active at creation time (e.g., whether encryption, compliance, or RAID were enabled), which discloses the volume's initial security posture to anyone who can read block 14. This is an accepted trade-off: the Recovery Block exists for disaster scenarios where all other metadata is inaccessible. **Guidance:** The Recovery Block MUST contain only the minimum information needed for disaster recovery identification (volume UUID, contact info, format version, block/inode sizes). It deliberately excludes encryption keys, key slot details, ACL policies, compliance passport contents, and any operational security metadata. The `ModuleManifest` copy is the creation-time value and is NOT updated when modules are added or removed online -- it reveals the initial module set only, not the current security configuration.
...
**ENOSPC race condition:** With thin provisioning and overcommit, two concurrent writers can both observe sufficient free space in the Allocation Bitmap (logical free blocks) and both attempt to allocate. If the underlying physical storage is nearly full, both allocations succeed logically but the second physical write fails with ENOSPC. **Mitigation:** The DW engine maintains a `PhysicalFreeEstimate` counter (updated atomically on every allocation) separate from the logical Allocation Bitmap. Before allocating a block, the allocator performs a two-phase check: (1) logical free bit in Allocation Bitmap, (2) `PhysicalFreeEstimate > 0` with atomic decrement via compare-and-swap. If the CAS fails (another thread consumed the last physical block), the allocator returns ENOSPC immediately without attempting the physical write. This serializes the physical capacity check without serializing the logical allocation path. The `PhysicalFreeEstimate` is periodically reconciled with actual filesystem free space via `statvfs()` / `GetDiskFreeSpaceEx()` by the Background Scanner.
...
| 9 | INCOMPAT_HAS_SMA | Secondary Metadata Area is present. The SMA uses the Inode Shadow Directory (ISD) model internally (AD-76): a flat array at SMA_BASE+4096, accessed via a userspace Radix Page Cache (AD-80), one 8-byte entry per inode, providing O(1) mathematical SMA addressing without any per-inode pointer in the primary inode format. OS-level mmap is explicitly prohibited for ISD access (see AD-80). |
...
+0x1C0  8     InnerVdeInodeNumber  uint64 LE: inode number of this VDE's object in the
                                   parent VDE (for reverse lookup). 0 if top-level.
                                   (Matches definitive Block 0 layout: uint64 at 0x01C0.)
```
...
The 3-bit QoS class is stored in the QOS module's inode field area, NOT in InodeFlags (which is a packed uint8 with no remaining bits). When the QOS module (bit 34) is active, the module's 4-byte inode field (in Module Overflow Block) encodes both TenantId and QoS class:
...
QUEUED (0x00) → INPROGRESS (0x01) → PAUSED (0x02) → INPROGRESS (0x01)
                    ↓    ↓               ↓
              COMPLETED  FAILED    CANCELLED (0x05)
               (0x03)    (0x04)
...
46. **Data Classification Tags** — Already covered by inline tags (176B InlineTagArea at inode offset 320, within the 184B TAGS module starting at offset 312).
...
| Tier 3 (Region/Superblock) | 10 | 13 (Set A: 3, Set B: 10) | 23 |
...
**Bottleneck analysis (50 MB, ~76ms typical):** Compression (33ms, ~43%) > VDE block write (16.8ms, ~22%) > AI inference (15ms GPU, ~20%) > PII + Auth (6.1ms, ~8%) > Consensus (2.5ms, ~3%). Percentages are approximate for the typical GPU/io_uring scenario. For CPU-only AI (30-80ms), AI dominates at 32-55% of the ~95ms total.

**Bottleneck analysis (4 KB, ~10ms typical with sync AI):** AI inference (6-9ms GPU, ~60-75%) > Raft quorum (1-3ms, ~15-25%) > Auth (0.5-1.2ms, ~8%). For small objects, everything except AI and consensus is sub-millisecond. With deferred AI mode (high-velocity), 4KB writes drop to **~3 ms**.
...
| Replication streaming | WAL tail read only | <1% P99 latency | io_uring/SPDK zero-copy DMA |
...
3. **AI inference is the #1 bottleneck for small objects.** Deferred mode (high-velocity ingest) skips sync AI classification entirely, reducing 4KB write from ~10ms to ~3ms. The B3.1 background job catches up within 1-5 minutes.

4. **All 124 features enabled adds ~3-5x overhead** vs raw IBlockDevice throughput. A raw 50MB NVMe write takes ~17ms; the full pipeline takes ~76ms. A raw 4KB NVMe write takes ~0.01ms; the full pipeline takes ~10ms (with deferred AI: ~3ms). The overhead is dominated by value-adding computation (compression, encryption, AI, integrity) rather than bureaucratic dispatch.

5. **Read overhead is lower (~1.7x for large objects)** because reads skip compression (decompress is faster), skip AI, skip consensus, and benefit from ARC caching. For 50MB objects: raw NVMe ~14ms vs full pipeline ~24ms = 1.7x. Metadata-only reads (inode + modules without data blocks) complete in ~1.5ms.
...
### Cost 1: Integrity Hashing (~1.5ms)
ext4 writes zero checksums. DW computes two integrity layers: (1) XxHash64 per block for TRLR entries (DataBlockTypeTag:4 + GenerationNumber:4 + XxHash64:8), and (2) BLAKE3 per extent for ExpectedHash verification. For 50MB: the BLAKE3 extent pass at ~4 GB/s takes ~1.2ms (dominates); XxHash64 at ~10 GB/s takes ~0.5ms (overlapped). Net cost: ~1.2ms hashing + ~0.3ms TRLR sequential write. This is the cost of per-block corruption detection that ext4 lacks entirely (and that even Btrfs does with weaker CRC32C).
...
**Superblock field:** `SmaStartBlock: uint64` at Superblock Block 0 offset 0x01C8 (first 8 bytes of the Reserved area). Value 0 = SMA disabled (default). Non-zero = absolute block number of the first (lowest-numbered) SMA block. The SMA grows toward lower block numbers (toward the primary area). When the SMA grows, `SmaStartBlock` decreases. **ISD Stability Invariant:** The ISD array is anchored at a fixed position within the SMA: it always starts at the block immediately after the SMA Header block, regardless of SMA growth. When the SMA grows, new SMA Data Area space is allocated between `SmaStartBlock` and the ISD — the ISD itself does not move. This ensures the ISD addressing formula `ISD_entry_address = ISD_BASE + (inode_num × 8)` remains stable across SMA growth operations, where `ISD_BASE` is the byte address of the ISD's first block (recorded in the SMA Header as a fixed block offset from the SMA Header block).
...
- **Shrink:** When SMA utilization drops below 20% for 7+ consecutive days, the Background Vacuum compacts the SMA and reclaims unused blocks back to the primary free pool. Compaction uses the `OwningInodeNum` reverse pointer (AD-83) in each SMA allocation for O(1) ISD entry relocation — no full ISD scan required.
...
Zone 2 — Inode Shadow Directory / ISD (bytes 4096+, inode_count × 8 bytes):
  ...
  An 800 MB ISD for 100M inodes consumes near-zero physical RAM via the
  userspace Radix Page Cache (AD-80) — only actively accessed 4KB pages are loaded.
...
Zone 3 — SMA Data Area (after ISD, variable):
  ...
  MANDATORY: Every allocation begins with a 12-byte header (AD-83):
  [OwningInodeNum:8][AllocationSize:4] followed by the payload.
  This enables O(1) reverse lookup AND sequential heap walking during SMA compaction.
```
...
0x50    16B   SmaIntegritySeal        HMAC-BLAKE3-128 (truncated to 128 bits) of SMA header fields 0x00-0x4F.
                                       128-bit truncation provides collision resistance equivalent to a 64-bit birthday bound
                                       (sufficient for corruption detection). For full tamper resistance, the Superblock's
                                       HeaderIntegritySeal (32B HMAC-BLAKE3-256) covers the SmaStartBlock pointer, providing
                                       256-bit tamper protection over the SMA's existence and location.
0x60    1B    IsdMode                 uint8: 0=Flat (AD-80 radix cache over flat array)
0x61    1B    Reserved61              Zero
0x62    8B    IsdHighWaterMark        uint64 LE — highest inode number with ISD coverage + 1 (AD-84)
0x6A    1B    InodeRecycleMode        uint8: 0=Immutable (default when REPL/XREF active), 1=Recycle (AD-84)
0x6B    3989B Reserved                Zero-filled (to end of 4096B header block)
```
...
**ISD Address Stability During SMA Growth:** When the SMA grows, new SMA Data Area blocks are allocated between `SmaStartBlock` (which decreases) and the existing SMA partition. The SMA Header block and ISD array do NOT move — they remain at their original positions near the end of the VDE. Only `SmaStartBlock` in the Superblock is updated to reflect the new lower boundary. This ensures the ISD addressing formula `ISD_entry_address = SMA_HEADER_BLOCK + 4096 + (inode_num × 8)` remains stable across all SMA growth operations. No ISD relocation, no cache invalidation, no I/O pause.

**WAL Checkpoint / Radix Cache Coupling (Implementation Requirement):** The WAL Checkpointer MUST NOT truncate WAL sequence N until all dirty ISD pages modified by transactions ≤ N have been physically flushed (fsync'd) from the Radix Page Cache to disk. Failure to enforce this invariant would allow a power failure to lose ISD updates that the WAL has already discarded, resulting in orphaned SMA allocations. This coupling is the standard WAL checkpoint protocol used by all WAL-based systems (PostgreSQL, SQLite, RocksDB).

**ENOSPC Rollback (Implementation Requirement):** When a VDE operates on thin-provisioned storage, the host filesystem may return ENOSPC on a write that the VDE's in-memory allocator approved. The io_uring/SPDK completion handler MUST detect ENOSPC failures and: (1) mark the VDE `READ_ONLY` via VdeState, (2) reverse the logical allocation in the bitmap, (3) emit a CRITICAL telemetry event. The in-memory PhysicalFreeEstimate should be periodically reconciled against host-reported free space (via `fstatfs`/`GetDiskFreeSpaceEx`) to reduce the frequency of this edge case.
...
**Decision:** The SMA partition uses a flat Inode Shadow Directory (ISD) as its first internal structure (starting at SMA_BASE + 4096, immediately after the SMA Header), accessed via a userspace Radix Page Cache (AD-80). The ISD provides O(1) mathematical addressing from any inode number to its SMA data, with O(1) orphan detection on delete. No per-inode pointer field is required in the primary inode format. InlineTagArea in the primary inode is restored to its full 176B.
...
void DeleteInode(ulong inodeNum)
{
    ...
    {
        // CRITICAL: Delete MUST be O(1). Zero the ISD entry FIRST, then defer cleanup.
        // This ensures the caller never blocks on overflow chain traversal.
        ulong oldOffset = entry.Offset;
        bool wasOverflow = entry.HasOverflow;
        uint oldSize = entry.InlineSize;

        UpdateIsdEntry(inodeNum, default);                  // zero the ISD entry (AD-78 page lock) — O(1)

        // Defer SMA block reclamation to the Background SMA GC queue.
        // The GC thread walks overflow chains and frees SMA blocks asynchronously.
        if (wasOverflow)
            SmaGcQueue.Enqueue(new SmaGcItem(oldOffset, SmaGcType.OverflowChain));  // O(1) enqueue
        else
            SmaGcQueue.Enqueue(new SmaGcItem(oldOffset, SmaGcType.InlineAlloc, oldSize)); // O(1) enqueue
    }
}
```

**O(1) Delete Guarantee:** The `DeleteInode()` function MUST complete in O(1) time regardless of SMA allocation complexity. The ISD entry is zeroed immediately (one radix cache update + page lock). SMA block reclamation — including overflow chain traversal for multi-year audit trails with 50+ descriptor links — is deferred to the **Background SMA GC Thread**.

**Background SMA GC Thread:** Drains the `SmaGcQueue` asynchronously. For inline allocations: single `FreeSmaBlocks()` call. For overflow chains: walks the linked descriptor list, frees each chunk, frees each descriptor. The GC thread runs at low priority (below foreground I/O) and batches frees for sequential NVMe efficiency.

**Crash Safety:** If the system crashes after zeroing the ISD entry but before the GC thread frees the SMA blocks, the blocks become orphans. The Background Scrubber (belt-and-suspenders) detects them during its periodic ISD cross-reference sweep and reclaims them. This is the same orphan recovery path used for crash-during-SMA-write scenarios (AD-77). No data corruption is possible — only temporary space leak until the scrubber runs.

```
Delete latency breakdown:
  RadixCache.ReadIsdEntry()  — ~50 ns (cache hit) / ~10 μs (cache miss + async DMA)
  UpdateIsdEntry()           — ~100 ns (page lock + CAS/write)
  SmaGcQueue.Enqueue()       — ~20 ns (lock-free MPSC queue)
  TOTAL                      — ~170 ns (hot path) / ~10.2 μs (cold path)

  Compare: synchronous 50-chunk overflow walk = 50 × 10 μs NVMe reads = ~500 μs MINIMUM
```

This eliminates the ZFS-style delete stall. The foreground thread returns in <1 μs. The background GC thread handles the chain walk without blocking any caller.
```
...
void UpdateIsdEntry(ulong inodeNum, IsdEntry newEntry)
{
    ...
    RadixCache.EnsurePageResident(pageIndex);               // ensure page in cache (may DMA — no lock held)
    lock (IsdPageLock[pageIndex])                           // acquire page lock (AD-80 lock ordering)
    {
        byte[] page = RadixCache.ReadPage(pageIndex);       // guaranteed cache hit after EnsurePageResident
...
**MANDATORY: Thread-Sharded Inode Allocation (Lock Contention Prevention):**

Inode numbers are allocated sequentially by default (AD-66). If N concurrent threads create files, they receive sequential inode numbers (e.g., 1000 through 1031). Because all 32 inodes fall within the same 512-entry ISD page, **all 32 threads slam into the same IsdPageLock** — converting the lock-free primary WAL into a single-mutex bottleneck.

The inode allocator MUST shard allocation ranges across threads:
```
Thread T allocates from range: [T × ShardStride, (T+1) × ShardStride)
ShardStride = ceil(max_inodes / ActiveThreadCount)

Example: 8 threads, 10M max inodes
  Thread 0: inodes 0 – 1,249,999       → ISD pages 0 – 2,440
  Thread 1: inodes 1,250,000 – 2,499,999 → ISD pages 2,441 – 4,882
  ...
  Thread 7: inodes 8,750,000 – 9,999,999 → ISD pages 17,089 – 19,531
```

With sharded allocation, concurrent SMA flushes hit **completely different ISD pages** — zero lock contention. The lock exists only as a safety guard for the rare case of cross-shard access (e.g., Background Scrubber touching pages from multiple shards).

**Interaction with AD-84 (Inode Recycling):** When recycling freed inode numbers, the allocator recycles within the thread's own shard range first. Cross-shard recycling is permitted but low-priority (only when the local shard is exhausted).

**Design Rationale:** A full SMA-specific WAL would guarantee atomicity at the cost of write amplification (every ISD update journals to WAL first, then applies). The per-page lock achieves the same correctness guarantee with zero write amplification — the ISD entry is written exactly once. The SMA write path remains journal-free as claimed in AD-77. The thread-sharded allocation ensures the lock is effectively uncontended under normal concurrent workloads.
...
0x10    8B    Chunk[0]             uint40 offset + uint23 size (same encoding as inline ISD entry bits 0-62, without overflow bit)
0x18    8B    Chunk[1]             ...
0x20    8B    Chunk[2]             ...
0x28    8B    Chunk[3]             ...
0x30    8B    Chunk[4]             ...
...
Each chunk uses the same `uint40 offset + uint23 size` encoding as the inline ISD entry (bits 0-39 for offset, bits 40-62 for size in 64B units, packed into 8 bytes with bit 63 unused/zero). A single overflow descriptor holds up to 6 chunks × 536 MB = 3.2 GB. The linked-list continuation via `Chunk[5]` as a `NextPtr` enables arbitrary chain lengths. Practical limit: a 100-descriptor chain supports 501 data chunks × 536 MB = ~262 TB per inode — far beyond any conceivable single-file SMA allocation. (99 non-terminal descriptors × 5 data chunks each [Chunk[5] used as NextPtr] = 495, plus the final terminal descriptor with 6 data chunks = 501 total data chunks.)
...
│  Coverage: 64^4 × 4KB = 64 TB addressable (exceeds 1 TB SMA)   │
├─────────────────────────────────────────────────────────────────┤
...
**Integration with AD-78 Page Locks:** The IsdPageLock array (AD-78) operates on logical page indices, not physical addresses. Whether a page is in the radix cache (hot) or must be fetched from disk (cold), the same lock index is used.

**MANDATORY Lock Ordering (Deadlock Prevention):** When updating an ISD entry, the caller MUST resolve the radix cache page FIRST (triggering DMA if needed on a cache miss and awaiting completion), THEN acquire the ISD page lock (AD-78). This ordering prevents a deadlock scenario where Thread A holds the page lock and blocks waiting for a DMA completion while Thread B needs the same page lock to complete its own update. The correct sequence is:
1. `RadixCache.EnsurePageResident(pageIndex)` — may issue async DMA, suspends caller until page is in cache. **No lock is held during the DMA wait.**
2. `IsdPageLock[pageIndex].Acquire()` — acquire the per-page mutex.
3. Read-Modify-Write the 4KB page in the radix cache.
4. `IsdPageLock[pageIndex].Release()` — release the mutex.

Read-only ISD lookups (no RMW) do NOT need the page lock — they only need the radix cache lookup (step 1). The page lock is required ONLY for write operations (AD-77 Step 3).

**Dirty Page Write-Back Ordering (AD-77 Compliance):** When the radix cache evicts a dirty ISD page (step 2 of the eviction policy above), the write-back MUST respect AD-77 ordering: the SMA payload data referenced by ISD entries on that page must already be persisted before the dirty ISD page is flushed. Because SMA payloads are written with an fsync barrier before the ISD entry is updated (AD-77 Steps 1-2-3), dirty ISD pages in the radix cache are guaranteed to reference already-persisted payloads. The eviction write-back therefore does NOT need an additional ordering barrier — AD-77's write-time ordering already guarantees correctness at eviction time.
...
### AD-83: SMA Allocation Reverse Pointer (O(1) Compaction)

**Decision:** Every SMA allocation in the SMA Data Area MUST begin with a 12-byte **Allocation Header** (`[OwningInodeNum:8][AllocationSize:4]`) before the payload data. This header enables the Background Vacuum to perform O(1) reverse lookups during SMA compaction — given any SMA allocation, the Vacuum can immediately determine which ISD entry points to it and update that entry after relocation.

**SMA Allocation On-Disk Layout (all allocations):**
```
Offset  Size  Field               Description
0x00    8B    OwningInodeNum      uint64 LE — the inode number that owns this SMA allocation
0x08    4B    AllocationSize      uint32 LE — total size of this allocation in bytes (header + payload)
0x0C    ...   Payload             Variable-length SMA data (vectors, lineage, compliance, etc.)
```

The 12-byte header (`OwningInodeNum` + `AllocationSize`) serves two mandatory purposes:

**Purpose 1 — O(1) Reverse Lookup (Compaction):** Given any SMA allocation, the Vacuum reads `OwningInodeNum` to immediately determine which ISD entry to update after relocation.

**Purpose 2 — Heap Walkability (Sequential Scan):** The `AllocationSize` field allows the Vacuum to sequentially walk the SMA Data Area as a heap: read header at offset X, advance to `X + AllocationSize` for the next allocation. Without `AllocationSize`, the Vacuum cannot determine where one allocation ends and the next begins — especially for **dead allocations** whose ISD entries have been zeroed. A dead allocation's `OwningInodeNum` may reference a freed inode, but its `AllocationSize` still allows the Vacuum to skip over it and find the next live allocation.

**Dead allocation detection:** During heap walk, the Vacuum reads `OwningInodeNum`, then checks `ISD[OwningInodeNum]`. If the ISD entry is zero (inode deleted) or points to a different offset (allocation was superseded), the allocation is dead — the Vacuum reclaims its space. If the ISD entry points to the current offset, the allocation is live.

**Space cost:** 12 bytes per SMA allocation (was 8 bytes with OwningInodeNum alone). For typical SMA allocations of 1-4 KB, this is 0.3-1.2% overhead — negligible.

**Why this is mandatory:**

Without the reverse pointer, SMA compaction is O(N²). The Background Vacuum's job during SMA shrink/defrag is:
1. Read an SMA allocation from a fragmented region
2. Copy the allocation to a contiguous free region
3. **Update the ISD entry** to point to the new location

Step 3 requires knowing which inode owns the allocation. Without `OwningInodeNum`, the Vacuum must scan the **entire ISD** to find the entry whose offset matches the old allocation address. For a 10B-inode ISD with 1,000 fragmented allocations to relocate, this is 1,000 × 10B = 10 trillion comparisons — a CPU death loop that would take hours.

With `OwningInodeNum`, Step 3 is:
```csharp
ulong ownerInode = ReadUInt64(allocationAddress);       // 8-byte read from the allocation header
IsdEntry entry = RadixCache.ReadIsdEntry(ownerInode);   // O(1) radix lookup
// ... copy payload to new location ...
UpdateIsdEntry(ownerInode, newOffset, entry.InlineSize); // O(1) update via AD-78 page lock
```

Total cost per relocation: two O(1) lookups + one sequential copy + one locked ISD write. SMA compaction is now O(M) where M = number of allocations to relocate, regardless of total inode count.

**Space cost:** 12 bytes per SMA allocation (8-byte `OwningInodeNum` + 4-byte `AllocationSize`). For typical SMA allocations of 1-4 KB (AI vectors, compliance passports), this is 0.3-1.2% overhead — negligible.

**Overflow descriptor integration (AD-79):** For overflow-chained allocations, each chunk in the overflow descriptor chain MUST also carry an `OwningInodeNum` header at the start of its payload. This allows the Vacuum to relocate individual chunks independently without traversing the entire chain.

**Integrity:** The `OwningInodeNum` is covered by the SMA allocation's own checksum (if checksummed by the owning module). The Background Scrubber validates `OwningInodeNum` consistency during its ISD cross-reference scan: if `ISD[N]` points to an allocation whose `OwningInodeNum != N`, a corruption event is logged and the orphan recovery path is triggered.

**Stale Pointer Defense (MANDATORY):** After an inode is deleted and its ISD entry zeroed, a buggy or malicious process with a cached old ISD value (stale offset+size) could attempt to read SMA data that now belongs to a different inode (use-after-free). The `OwningInodeNum` field in the Allocation Header serves as a **reverse-pointer guard**: when reading SMA data for inode N, the engine MUST verify that `Allocation.OwningInodeNum == N` before returning the payload. If the check fails, the SMA data does not belong to the requesting inode — return an error and trigger orphan recovery. This makes stale-pointer attacks detectable at O(1) cost (one uint64 comparison per SMA read). Combined with the inode allocation bitmap check (is inode N still allocated?), this provides defense-in-depth against both stale caches and use-after-free scenarios.

---

### AD-84: ISD High-Water Mark and Inode Recycling

**Decision:** The ISD is a mathematical flat array indexed by inode number. By construction, it cannot be shrunk without renumbering inodes — which would break external application handles (file descriptors, NFS file handles, FUSE inode numbers), Fabric Links (`dw://` URIs), replication DVV references, and snapshot inode mappings. This is an **accepted, permanent trade-off** of O(1) mathematical addressing.

**The problem:** A multi-tenant VDE that creates 1 billion ephemeral files reaches ISD high-water mark of 8 GB. If 999,999,999 files are deleted, only inode #1,000,000,000 remains active, but the ISD remains 8 GB on disk because the address formula `SMA_BASE + 4096 + (inode_num × 8)` requires the array to extend to entry 1,000,000,000.

**Mitigation 1 — Immutable Inode Numbers (MANDATORY for Federation / Replication):**

In a distributed Data Warehouse (Stage 7 Federation), inode numbers are **external identity anchors**. A `dw://` Fabric Link on another continent, a replication DVV vector clock, an NFS file handle, or a snapshot inode mapping all reference inodes by number. If inode 1000 is deleted and recycled, the new file silently inherits the old file's distributed identity — all remote references now point to incorrect data. This is a **federation split-brain** that cannot be detected or recovered.

**Rule:** Inode numbers MUST NEVER be recycled when ANY of the following features are active:
- Replication (`REPL` module, bit 5)
- Fabric Links / Cross-VDE References (`XREF` module, bit 12)
- Federation Router (Stage 7)
- Snapshots with external consumers (`SNAP` module, bit 7, with `ExternalSnapshotHandles > 0`)

When inode recycling is prohibited, the allocator always extends the ISD high-water mark:
```
Inode Allocation Algorithm (Immutable Mode):
1. new_inode = IsdHwm        // always the next sequential number
2. IsdHwm += 1               // extend ISD
3. Return new_inode
```

This accepts the sparse ISD bloat as the cost of distributed referential integrity. Because the AD-80 Radix Page Cache only loads accessed pages, an 80 GB sparse ISD with 1M active inodes costs **exactly ~8 MB of RAM** — the 79.99 GB of empty ISD pages are never loaded.

**Mitigation 1b — Low-Water Inode Recycling (ONLY for Standalone, Non-Replicated VDEs):**

For standalone VDEs with no replication, no Fabric Links, and no federation (Stages 1-4 without REPL/XREF), inode recycling IS safe and SHOULD be used to limit ISD growth:

```
ISD High-Water Mark (IsdHwm): stored in SMA Header at offset 0x62 (8 bytes)

Inode Allocation Algorithm (Recycling Mode):
1. Consult the Inode Free List (bitmap scan) within the local thread shard (AD-78)
2. If lowest_free_in_shard < IsdHwm: allocate it (reuses existing ISD space)
3. If no free inodes in shard below IsdHwm: extend ISD, update IsdHwm
```

**Mode selection:** `InodeRecycleMode: uint8` at SMA Header offset 0x6A. Values: 0 = Immutable (default when REPL or XREF active), 1 = Recycle (default for standalone). Mode transition from Recycle to Immutable is instant (just stop recycling). Transition from Immutable to Recycle is prohibited once any external reference exists.

**CRITICAL: GC-Recycling Race Prevention (MANDATORY when InodeRecycleMode = 1):** When inode N is deleted, `DeleteInode()` zeroes the ISD entry and enqueues the SMA blocks for background GC (see AD-76 delete path). If the inode allocator recycles inode number N before the GC thread frees the old SMA blocks, and the new inode writes SMA data at the same SMA offset (reused by the SMA free-space allocator), the GC could incorrectly free the new inode's SMA data.

**Prevention:** The `SmaGcQueue` maintains a `PendingRecycleSet` (hash set of inode numbers with pending GC). The inode allocator MUST check this set before recycling: if `PendingRecycleSet.Contains(inodeNum)`, skip this inode and try the next free one in the shard. The GC thread removes entries from `PendingRecycleSet` after freeing the SMA blocks. Cost: O(1) per allocation (hash set lookup). The `OwningInodeNum` reverse-pointer guard (AD-83) provides belt-and-suspenders defense: even if the race occurred, the stale `OwningInodeNum` would be detected on the next SMA read.

**Mitigation 2 — ISD Tail Truncation (Background, Opportunistic, Standalone Only):**

ISD tail truncation is ONLY permitted when `InodeRecycleMode = 1` (Recycle). When the highest allocated inode number drops significantly below the ISD high-water mark, the Background Vacuum MAY truncate the ISD tail:

```
if highest_allocated_inode < IsdHwm * 0.5:     // active inodes use less than half the ISD
    new_hwm = highest_allocated_inode + PADDING  // PADDING = 10% growth headroom
    AcquireInodeAllocationFreeze()               // prevent concurrent allocations during truncation
    verify no inode above new_hwm is allocated   // sequential bitmap scan — O(N/8) bytes
    zero ISD entries from new_hwm to IsdHwm      // sequential zero-fill
    update IsdHwm = new_hwm
    ReleaseInodeAllocationFreeze()
    // SMA allocator can now reclaim the freed ISD tail space
```

**Constraint:** The `AcquireInodeAllocationFreeze()` prevents the TOCTOU race between bitmap verification and truncation. The freeze is held only for the duration of the bitmap scan + zero-fill — typically milliseconds for the tail region. Concurrent data I/O is NOT blocked, only new inode allocations.

**Mitigation 3 — Radix Cache Indifference (RAM Cost = Zero for Empty Ranges):**

Regardless of on-disk ISD size, the Radix Page Cache (AD-80) only holds pages that have been accessed. An 8 GB ISD with 1M active inodes spread across the low end consumes ~8 MB of Radix Cache pages — the remaining 7.99 GB of empty ISD pages are never loaded, never cached, and consume zero RAM. The on-disk bloat is a storage cost, not a memory cost.

**SMA Header updates:** Add fields at offsets 0x62 and 0x6A:
```
0x62    8B    IsdHighWaterMark    uint64 LE — highest inode number with ISD coverage + 1
0x6A    1B    InodeRecycleMode    uint8: 0=Immutable (default when REPL/XREF active), 1=Recycle
```
This reduces the Reserved area from 3998B to 3989B (0x6B-0xFFF, filling the remainder of the 4096B SMA Header block).

---

### AD-85: TRLR Crash Atomicity and Data WAL Integration

**Decision:** NVMe controllers do NOT guarantee atomic persistence for multi-block I/O submissions, including SGL-based writes. A power failure during a 256-block write (255 data + 1 TRLR) can persist some data blocks while dropping the TRLR block, creating a 1 MB "blast radius" — up to 255 data blocks with no integrity metadata.

This is not a flaw unique to DWVD; it affects every storage system that uses multi-block writes. The specification MUST explicitly document the recovery guarantees and the Data WAL's role in preventing data loss.

**The Blast Radius Problem:**

```
Write submission: [Data₀][Data₁]...[Data₂₅₄][TRLR]
                   ↑ persisted ↑              ↑ lost (power failure mid-DMA)

Result: Data₀ through Data₂₀₀ are on disk but their TRLR block is missing.
        XxHash64 checksums for those 200 blocks are gone.
        GenerationNumber for those blocks is gone.
```

Without the TRLR block, the data blocks cannot be verified. They are physically present but integrity-orphaned.

**Recovery Strategy — Three Tiers:**

**Tier 1 — Data WAL Active (Default for Durability-Critical Workloads):**

When the Data WAL (`DWAL` region) is active, every data write is journaled before being written to the Data Region:
```
1. Write data blocks + TRLR records to Data WAL (sequential, single NVMe submission)
2. fsync / NVMe FLUSH barrier (guarantee WAL persistence)
3. Write data blocks to Data Region (final location)
4. Write TRLR block to Data Region
5. Advance WAL checkpoint (marks WAL entries as committed)
```

If power fails between steps 3 and 4, recovery replays the WAL:
- The WAL contains both the data blocks AND their TRLR records
- Recovery writes the missing TRLR block from the WAL to its deterministic location
- Zero data loss, zero integrity orphans

**Trade-off:** Write amplification = 2× (data written to WAL, then to Data Region). For write-heavy workloads, this is significant. The Data WAL is configured per-VDE and can be disabled for workloads that tolerate the blast radius (see Tier 2).

**Tier 2 — Data WAL Disabled, TRLR Recovery via ExpectedHash (Reduced Durability):**

For throughput-critical workloads (streaming ingest, batch analytics) where 2× write amplification is unacceptable:
```
Data WAL disabled: dw config set data-wal=off
```

If a TRLR block is lost, recovery detects the gap (the TRLR block's own 16B trailer will fail XxHash64 verification or be zero-filled). Recovery then:
1. Reads each data block in the affected 255-block stripe
2. For each data block, verifies `ExpectedHash:16` (BLAKE3) from the extent pointer to detect torn writes (partially written blocks). If BLAKE3 verification fails, the block suffered a torn write — mark it as corrupt and exclude from TRLR reconstruction
3. Recomputes `XxHash64` for each verified-intact data block from its raw contents. Torn-write blocks receive `XxHash64 = 0` and `IS_CORRUPT` flag in their TRLR entry
4. Reconstructs the TRLR block with recomputed checksums
5. Writes the reconstructed TRLR block to its deterministic location
6. **fsync / NVMe FLUSH barrier** — guarantees the reconstructed TRLR block is persisted before recovery proceeds. Without this barrier, a second crash during recovery could lose the reconstructed TRLR, forcing re-recovery of the same stripe
7. Sets `GenerationNumber` to the current recovery epoch (loss of original generation — CBT accuracy for this stripe is degraded)

**Torn Write Handling (Partial Data Block Writes):** A power failure during a multi-block write can produce torn writes — data blocks where the 4KB sector is partially overwritten. Tier 2 detects torn writes via the extent-level BLAKE3 check in step 2. If `BLAKE3(data_block) != extent.ExpectedHash`, the block is torn. If the extent has RAID parity, recovery falls through to Tier 3 for reconstruction. Without RAID, the torn block is irrecoverable — recovery marks the extent as damaged, logs a `DataIntegrityError` event, and continues. The application receives an I/O error on next read of the affected extent. This is the accepted trade-off for disabling the Data WAL.

**What's lost:** The original `GenerationNumber` values. This means incremental backup (AD-49) may re-backup these blocks unnecessarily (one extra backup cycle). The data itself is intact for non-torn blocks — `XxHash64` is recomputed, not lost. Torn blocks (BLAKE3 mismatch) are irrecoverable without RAID.

**What's NOT lost (for intact blocks):** The data blocks themselves are physically intact. `ExpectedHash:16` (BLAKE3) in the extent pointers provides a second integrity check independent of the TRLR — even with a missing TRLR, the extent-level BLAKE3 verifies data correctness and detects torn writes.

**Tier 3 — RAID Reconstruction (Maximum Resilience):**

When Polymorphic RAID is active (RAID module enabled, e.g., EC_4_2 erasure coding):
- The TRLR block is just another 4 KB block within the RAID stripe
- RAID reconstruction can regenerate a lost TRLR block from parity data
- Zero data loss, zero generation loss, zero integrity impact
- This is the recommended configuration for enterprise deployments

**Default Configuration:**

| Deployment Profile | Data WAL | RAID | Blast Radius | Write Amplification |
|-------------------|----------|------|-------------|-------------------|
| Enterprise (default) | ON | EC_4_2 | 0 (WAL + RAID) | 2× |
| Analytics / Streaming | OFF | EC_4_2 | 0 (RAID reconstructs TRLR) | 1× |
| Edge / IoT | OFF | None | 1 MB (255 blocks) | 1× |
| Maximum Durability | ON | EC_4_2 | 0 (triple redundancy) | 2× |

**Specification mandate:** The VDE MUST log a warning at mount time if both Data WAL and RAID are disabled:
```
WARNING: Data WAL and RAID are both disabled. A power failure during write
         can orphan up to 255 data blocks (1 MB) per incomplete TRLR group.
         Data is recoverable via XxHash64 recomputation but GenerationNumber
         history will be lost for the affected stripe.
```
...
**SMA:** Still disabled for most deployments. The 184B TAGS module (InlineTagCount:4 + TagOverflowBlock:4 + 176B InlineTagArea) within the primary inode handles ~85% of tagging needs (5 full 32B tags + 16B remainder that can hold a compact 6th tag). Extended metadata features (AI vectors, compliance vaults, search indexes) are not needed at this scale.
