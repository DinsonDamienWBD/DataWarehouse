...
0x6B    917B  Reserved                Zero-filled
```
...
**Space cost:** 8 bytes per SMA allocation. For typical SMA allocations of 1-4 KB (AI vectors, compliance passports), this is 0.2-0.8% overhead — negligible.
...
```
This reduces the Reserved area from 928B to 917B (0x6B–0xFFF).
...
2. Recomputes `XxHash64` for each block from its raw contents
3. Reconstructs the TRLR block with recomputed checksums
4. Writes the reconstructed TRLR block to its deterministic location
5. Sets `GenerationNumber` to the current recovery epoch (loss of original generation — CBT accuracy for this stripe is degraded)

**What's lost:** The original `GenerationNumber` values. This means incremental backup (AD-49) may re-backup these blocks unnecessarily (one extra backup cycle). The data itself is intact — `XxHash64` is recomputed, not lost.

**What's NOT lost:** The data blocks themselves, which are physically intact. `ExpectedHash:16` (BLAKE3) in the extent pointers provides a second integrity check independent of the TRLR — even with a missing TRLR, the extent-level BLAKE3 can verify data correctness.
