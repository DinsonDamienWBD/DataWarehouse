...
Zone 3 — SMA Data Area (after ISD, variable):
  ...
  MANDATORY: Every allocation begins with an 8-byte OwningInodeNum header (AD-83)
  followed by the payload. This enables O(1) reverse lookup during SMA compaction.
```
...
0x6A    918B  Reserved                Zero-filled
```
...
0x08    ...   Payload             Variable-length SMA data (vectors, lineage, compliance, etc.)
```
...
**Mitigation 1 — Low-Water Inode Recycling (Default):**

The inode allocator MUST prefer recycling freed inode numbers below the current high-water mark before extending the ISD. The mechanism:

```
ISD High-Water Mark (IsdHwm): stored in SMA Header at offset 0x62 (8 bytes, new field in Reserved area)

Inode Allocation Algorithm:
1. Consult the Inode Free List (bitmap scan) for the LOWEST available inode number
2. If lowest_free < IsdHwm: allocate it (reuses existing ISD space, no ISD growth)
3. If lowest_free >= IsdHwm: extend ISD, update IsdHwm
```

This ensures that the ISD only grows when ALL inode numbers below the high-water mark are in use. In the pathological case above, after deleting 999,999,999 files, the next 999,999,999 allocations will reuse inode numbers 0 through 999,999,998 — the ISD never grows beyond 8 GB. Over time, if the active inode count stabilizes at 1M, the Radix Page Cache will only hold pages for the ~1M active inodes regardless of the 8 GB on-disk ISD.

**Mitigation 2 — ISD Tail Truncation (Background, Opportunistic):**

When the highest allocated inode number drops significantly below the ISD high-water mark, the Background Vacuum MAY truncate the ISD tail:
...
**Constraint:** Tail truncation is ONLY safe when no inode number above `new_hwm` is allocated. The Vacuum MUST scan the inode allocation bitmap to confirm this before truncating. This is a sequential bitmap scan — O(N/8) bytes read, no random I/O.
...
**SMA Header update:** Add field at offset 0x62:
```
0x62    8B    IsdHighWaterMark    uint64 LE — highest inode number with ISD coverage + 1
```
This reduces the Reserved area from 928B to 920B (0x6A–0xFFF).
