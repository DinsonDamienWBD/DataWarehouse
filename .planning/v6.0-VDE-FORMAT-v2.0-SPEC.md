...
1. Magic signature conflict resolved — `DWVD-BOO` (8B truncation of "DWVD-BOOT") occupies bytes 0-7; `DWVD` magic still resides at `vde_offset` within the format.
...
On a host OS: the VDE engine reads bytes 0-7. Detects `DWVD-BOOT` → reads preamble header → seeks to `vde_offset` → mounts Block 0 as normal. The preamble bytes are never mapped into any VDE region and consume no VDE capacity.
...
if bytes[0..7] == "DWVD-BOO":   // 8-byte truncation of "DWVD-BOOT"
...
Tools and file identification utilities SHOULD check for both `DWVD-BOOT` (bytes 0-7) and `DWVD` (bytes 0-4). The canonical format written by the DW engine is always `DWVD-BOOT` prefixed. The `DWVD` magic survives at `vde_offset` within the file for backward-compatible format probing.
...
Block Type Tags: SUPB, RMAP, POLV, ENCR, BMAP, INOD, TAGI, MWAL, MTRK, BTRE, SNAP, REPL, RAID, INTE, STRE, XREF, WORM, CODE, DWAL, DATA, TRLR, FREE, RSTA, OPJR, CMVT, ALOG, CLOG, DICT, MLOG, WALS, ZNSM, RCVR
