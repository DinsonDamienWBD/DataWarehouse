...
35   VectorQuantize     VECQ    —                             0    Vector quantization for embedding compression (overflow)
36   AncestryChaining   ANCR    —                             0    Cryptographic ancestry back-pointers (overflow)
37   CapabilityMacaroon MACR    —                             0    Macaroon-based delegated capabilities (overflow)
38   WormTimeLock       WLCK    —                             0    Time-locked WORM with cryptographic proof (overflow)
...
Stored in Superblock Block 2 (Extended Metadata), within the free space after existing extended metadata fields. Block 1 is fully consumed by 127 Region Pointer slots (127 × 32 = 4064 bytes + 16B trailer = 4080B), leaving no room for the descriptor without a block-boundary crossing that would corrupt the Universal Block Trailer.
...
**Block 2 space budget:** Existing extended metadata fields consume 1280B. Backup Manifest adds 128B. Recovery Bookmarks add 512B (8 × 64B, with 8B unused padding per slot). InodeLayoutDescriptor adds up to 281B (8B header + 39 × 7B module entries). Total used: 2201B of 4080B usable. Remaining: 1879B available for future extensions.
