...
~50+ fields including: Magic, FormatVersion, BlockSize, TotalBlocks, FreeBlocks, VolumeUUID, ClusterNodeId, DefaultCompressionAlgo, DefaultEncryptionAlgo, DefaultChecksumAlgo, InodeSize(512), PolicyVersion, ReplicationEpoch, WormHighWaterMark, EncryptionKeyFingerprint, SovereigntyZoneId, VolumeStateFlags, VolumeLabel, LastScrubTimestamp, ErrorMapBlockCount
...
320     180    InlineTagArea            Up to ~6 compact tags:
                                        Each: [NamespaceHash:4][NameHash:4][ValueType:1][ValueLen:1][Value:≤22B]
                                        Eliminates overflow I/O for ~85% of tagged objects.

── DataOsModuleArea (16 bytes, fixed-offset layout) ──
500     2      [SPOL] module            (if ActiveModules bit 27 set):
                                          ...
                                          Bits 9-15: reserved
502     8      [DELT] module            (if ActiveModules bit 22 set):
                                          [MaxDeltaDepth:2][CurrentDepth:2][CompactionPolicy:4]
510     6      [STEX] module            (if ActiveModules bit 24 set):
...
- InlineTagArea expanded to 180 bytes (from 128B) — fits ~6 enterprise tags inline, dramatically reducing TagIndexRegion I/O
...
Bits 6-7:  QoS_Class              2 bits, NVMe hardware priority class
...
- QoS_Class reduced from 4 bits to 2 bits (4 classes match NVMe hardware queue priorities).
...
34   QoS                QOS     —                             2    Per-inode QoS class (3 bits in InodeFlags bits 5-7) + TenantId (2B overflow).
...
The 32-bit `FeatureFlags` field is retired. Module-backed features are already tracked by `ModuleManifest` — maintaining a separate flags field for the same information creates split-brain consistency bugs.
...
Region Directory overflow: Slot 64 can point to an overflow block (RDIR_OVERFLOW). Overflow blocks chain via their own last slot. Maximum chain supports 256+ core machines.
...
+0x02   2     CoreFieldsEnd      uint16 LE: byte offset where core fields end.
                                 Always 304 in v2.1 (identity + timestamps +
...
**Gate:** QOS module (bit 34) must be active. When inactive, bits 5-7 are reserved/zero and the I/O scheduler uses default priority for all operations.
