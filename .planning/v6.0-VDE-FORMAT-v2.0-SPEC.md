...
Tools and file identification utilities SHOULD check for both `DWVD-BOOT` (bytes 0-7) and `DWVD` (bytes 0-3). The canonical format written by the DW engine is always `DWVD-BOOT` prefixed. The `DWVD` magic survives at `vde_offset` within the file for backward-compatible format probing.
