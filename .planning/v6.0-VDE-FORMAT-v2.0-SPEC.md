...
#### Hierarchical Free-Space Index (AD-68)

The flat Allocation Bitmap is efficient for VDEs ≤100 TB (~3.2 GB bitmap, fits in RAM). For PB+ scale deployments, a supplementary B-tree free-space index is built on top of the bitmap shards:

**Structure:** Each bitmap shard maintains a B-tree of free extents: `[StartBlock:8][BlockCount:8]` sorted by BlockCount (largest-first) and secondarily by StartBlock. The B-tree is stored in dedicated blocks within the bitmap region.

**Allocation fast path:**
1. Query the free-extent B-tree for the shard: `FindFirstFit(requestedBlocks)` → O(log N)
2. Split the free extent, update B-tree → O(log N)
3. Mark bits in the flat bitmap (authoritative source of truth) → O(1) with known offset

**Deallocation:** Mark bits free in the bitmap. Queue a B-tree insertion for the Background Vacuum to coalesce adjacent free extents.

**Scaling:**
| VDE Size | Bitmap Size | B-Tree Free Extents | Allocation Cost |
|----------|-------------|-------------------|-----------------|
| 1 TB | 32 MB | ~few thousand | O(1) bitmap scan |
| 100 TB | 3.2 GB | ~millions | O(log N) B-tree |
| 1 PB | 32 GB | ~tens of millions | O(log N) B-tree |

**Fallback:** If the B-tree is stale or unavailable (corruption, first mount), the engine falls back to sequential bitmap scanning. The flat bitmap remains the authoritative free-space record; the B-tree is an acceleration index rebuilt lazily by the Background Vacuum.

This is architecturally equivalent to ZFS metaslabs and XFS AG free-space B-trees. The flat bitmap provides crash-recovery simplicity (every bit is authoritative); the B-tree provides O(log N) allocation performance at scale.
...
Decision: DW's VDE layer (Phase 6) is latency-competitive with ext4, NTFS, Btrfs, and ZFS for equivalent I/O operations. The full-stack pipeline adds ~59 ms of value-adding computation (compression, encryption, AI, consensus, compliance) that traditional filesystems do not provide. To achieve equivalent feature coverage with ZFS requires 11+ additional products, resulting in ~1.6x higher latency (122 ms vs 76 ms) and dramatically higher operational complexity. DW's architectural advantage is the integrated single-process pipeline with shared caching, eliminating the IPC and redundant-cache penalties of a bolt-on approach. A bare DW VDE with ModuleManifest=0 operates at ext4-competitive latency (~17ms for 50MB); the moment any enterprise feature is needed, DW's integrated pipeline outperforms the equivalent bolt-on stack by 1.3-1.6x because disabled modules cost exactly zero — one bit check per pipeline stage — while bolt-on products impose fixed IPC, caching, and process management overhead regardless of utilization.
