...
data_physical_offset = logicalDataBlockIndex + floor(logicalDataBlockIndex / 255) + (1 if logicalDataBlockIndex >= 255 else 0)
```
...
`COMPLEX_FLAGS_MASK` covers bits 3-5 and 8-15 (flags requiring algorithmic work: Shared_COW copy-on-write resolution, Delta_Extent, Torn_Write_Boundary, RAID, crypto offload, etc.). Bits 0-2 and 6-7 (compression class, QoS) are simple O(1) table lookups. Note: the 3-tier XOR dispatch applies to the **READ** path. WRITE operations have a separate path that handles Shared_COW allocation and reference counting.
...
Maximum module payload for ALL 39 modules simultaneously: ~1236B (261B inline inode module fields + 16B DataOsModuleArea + ~959B overflow modules). Two chained overflow blocks (968B) handles the v6.0 subset (~448B); three blocks (1440B available) are needed for the full v7.0 theoretical maximum (~1236B).
...
When `SPATIOTEMPORAL` (InodeFlags bit 5) is set, the inode's 6 × 32B extent slots are reinterpreted as 3 × 64B spatiotemporal extents:
```
...
**Verification:** 16 + 48 + 32 + 16 + 48 + 24 + 40 + 32 + 20 + 28 + 64 + 16 + 8 + 36 + 24 + 3596 + 32 + 16 = 4096 bytes exact. Defined section fields = 456B (includes 8B InnerVdeInodeNumber), HMAC seal = 32B, TRLR = 16B, Reserved padding = 3592B, total = **4096 bytes exact** ✓
...
ModuleManifest:   0x0000005DF9FFFFFF (all 36 v6.0-implementable modules active)
...
ModuleConfigExt:  0xF0F0FFFFFFFFFFFF (modules 16-24 at 0xF, 25-26 at 0x0, 27-31 at 0xF)
ModuleConfigExt2: 0x00000000F0FFFFFF (module 32 at 0xF, 33 at 0x0, 34-38 at 0xF, 39-47 at 0x0)
...
Each chunk uses the same `uint40 offset + uint23 size` encoding as the inline ISD entry (bits 0-39 for offset, bits 40-62 for size in 64B units, packed into 8 bytes with bit 63 unused/zero). A single overflow descriptor holds up to 6 chunks × 536 MB = 3.2 GB. The linked-list continuation via `Chunk[5]` as a `NextPtr` enables arbitrary chain lengths. Practical limit: a 100-descriptor chain supports 599 chunks × 536 MB = ~313 TB per inode — far beyond any conceivable single-file SMA allocation. (99 descriptors with 6 full chunks = 594, plus the final descriptor with 5 chunks and `Chunk[5]` as `NextPtr = NULL` = 599 total chunks.)
