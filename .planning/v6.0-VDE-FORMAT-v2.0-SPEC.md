...
| 9-31 | Reserved | Future incompatible features |
...
| **DW VDE (Golden 14)** | all Golden features, no compress/RAID | **~17 ms** | **~2.9 GB/s** | + encryption, snapshots, epochs, TRLR, tags, replication, journal, observability, policy — 11 structurally free features piggybacking on existing I/O. Same NVMe operation count as bare VDE. |
...
Golden 14 (no compress/    ~17 ms                 + encryption, snapshots, epochs, TRLR, tags,
  RAID — all free)                                  replication, journal, observability, policy,
                                                    integrity levels, DR — 11 structurally free
                                                    features + 20+ emergent features. SAME I/O
                                                    COUNT AS BARE STORAGE.
...
The Golden 14 are chosen not for "acceptable latency cost" but because **11 of 14 add zero I/O operations**, and the remaining 3 are explicit user choices that trade I/O for value (compression reduces total blocks, RAID adds parity blocks, sync replication adds network round-trips). With compression and RAID disabled, a DW VDE with all 14 Golden features performs the **same number of NVMe operations as raw ext4** — but every block is self-verifying, encrypted, temporally-ordered, and snapshot-aware.
...
|----|---------|---------------|-------------|-------------|
| G1 | TRLR (Trailers) | Last 16B of every 4KB data block | Block is already being written. 16 bytes at the end of the same NVMe page. | Per-block type tag, generation counter, XxHash64 checksum. Backbone of 20+ emergent features. |
...
| G4 | Encryption | Transforms data bytes in-place | Same blocks, same count, same size — just different bytes. AES-256-GCM with AES-NI at ~10 GB/s pipelines with NVMe writes (encrypt block N while NVMe writes block N-1). | At-rest encryption. Regulatory table stakes. No separate dm-crypt/LUKS layer needed. |
...
**11 features × 0 additional I/O = ext4-equivalent I/O count with 11 enterprise features included.**
...
#### What This Means: DW vs ext4 at Identical I/O
...
| DW (Golden 14, no compress/RAID) | Data blocks + inode + journal | Self-verifying (XxHash64), encrypted (AES-256-GCM), temporally-ordered (epoch), snapshot-aware, tagged, policy-governed, observable. Plus 20+ emergent features. |

**Same I/O count. Same NVMe operations. Radically richer per-block data.**
...
**14 baked features (11 free + 3 opt-in) → 20+ emergent features → 34+ total capabilities.**
...
**Hot-path latency:** **~17 ms** (identical to ext4+LUKS) with 11 structurally-free features active.
...
| **Golden Path** | **14 (11 free + 3 opt-in)** | **~17 ms** | **20+** | **1** |
...
Decision: The Golden 14 features are selected because 11 of 14 add zero additional I/O operations — they piggyback on data blocks, inodes, and journal entries the filesystem is already writing. The remaining 3 (compression, RAID, sync replication) are explicit user opt-ins that trade I/O for value. With compression and RAID disabled, a DW VDE with all 14 Golden features active performs the same number of NVMe operations as raw ext4, but every block is self-verifying, encrypted, temporally-ordered, and snapshot-aware. Mathematical composition of the Golden 14 produces 20+ emergent features at zero cost. The competitive position is not "acceptable overhead for enterprise features" — it is "enterprise features at zero overhead, with emergence as a bonus." Features beyond the Golden 14 (AI vectors, compliance vaults, lineage, search indexes) are served by the Secondary Metadata Area (AD-70) and add latency only when explicitly requested.
...
**SMA Header (1 block, 4096B):** See "SMA Header Extended Fields" below for full layout including backfill state and runtime configuration.
...
Each inode contains a single field for SMA linkage:

```
Inode offset 0x1F0:  SmaBlockPtr  uint64 LE
```

- Value 0 = no SMA metadata for this inode (default, common case)
- Non-zero = block address within the SMA where this inode's extended metadata begins

The SMA block pointed to by `SmaBlockPtr` contains a mini-directory:
```
Offset  Size  Field              Description
0x00    4B    Magic              "SMAL" (0x534D414C) — SMA Linkage
0x04    2B    EntryCount         uint16 LE — number of region entries
0x06    2B    Reserved           Zero
0x08    N×16B Entries            [RegionTypeCode:4][Reserved:4][BlockOffset:8] per entry
```

This means: for a normal `read()` or `write()`, the engine sees `SmaBlockPtr == 0` and skips entirely. Zero overhead. Only when the user explicitly asks for extended metadata does the engine follow the pointer.
...
Normal `read(fd, buf, len)` NEVER touches the SMA. The engine reads: inode (primary) → extent pointers → data blocks (primary) → return to application. The `SmaBlockPtr` field is not even examined during normal reads.

SMA reads occur only for explicit metadata queries:
- `dw vectors /file` → follow SmaBlockPtr → read VECR region entry → return vectors
- `dw lineage /file` → follow SmaBlockPtr → read LNGR region entry → return lineage graph
- `dw compliance /file` → follow SmaBlockPtr → read CMVR region entry → return compliance passport

Each SMA read is a single additional I/O (the SMA linkage block) plus the region-specific data. Latency: ~0.1ms for the linkage block + variable for the region data.
...
6. VDE is immediately usable — SMA regions are populated lazily as data is written and extended features process it
...
6. **SMA is now active.** All new writes immediately start queueing SMA metadata for background flush.
...
Backfill Pipeline (background, throttled):
  for each inode in Inode Table (sequential scan):
    if inode.SmaBlockPtr != 0:
    ...
    // Write to SMA
    allocate SMA linkage block
    write region entries to SMA

    // Update primary inode (atomic)
    set inode.SmaBlockPtr = linkage block address
    journal the inode update via Metadata WAL
...
- **Non-blocking:** Foreground `read()`/`write()` operations are never delayed by backfill. The backfill thread yields whenever primary I/O queue depth exceeds a configurable threshold (default: 50% of NVMe queue capacity).
...
After backfill completes, the SMA is fully populated. All inodes have `SmaBlockPtr` set (either from backfill or from normal write-time population). The engine transitions to steady-state SMA operation: lazy writes for new/modified data, on-demand reads for metadata queries.
...
**Drain** exports all SMA metadata to an external sidecar file (`.dwvd.meta`). This preserves the metadata for future re-enablement. **Disable** then zeroes all `SmaBlockPtr` fields in inodes (background, same pattern as backfill), reclaims SMA blocks to the primary free pool, and clears the superblock flag.
...
The SMA Header includes fields for backfill state and runtime configuration:
...
0x30    8B    BackfillHighWaterInode  uint64 LE — last inode processed by backfill (0 = not started, MAX = complete)
0x38    1B    BackfillState           uint8: 0=NotStarted, 1=Running, 2=Paused, 3=Complete
0x39    1B    SmaCapPercent           uint8 — configured cap as % of VDE (1-100, default 5)
0x3A    2B    EnabledSmaModules       uint16 LE — bitmask of active SMA region types
0x3C    4B    BackfillRatePerSec      uint32 LE — throttle rate (inodes/sec, default 1000)
0x40    16B   SmaIntegritySeal        HMAC-BLAKE3 of SMA header fields 0x00-0x3F
0x50    944B  Reserved                Zero-filled
```
