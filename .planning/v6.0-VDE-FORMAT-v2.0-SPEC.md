...
Effective payload: BlockSize - 16 = 4080 bytes at 4 KiB. Overhead: 0.39%.

Benefits: Every block self-verifiable, offline forensic recovery without metadata, torn-write detection.

Block Type Tags: SUPB, RMAP, POLV, ENCR, BMAP, INOD, TAGI, MWAL, MTRK, BTRE, SNAP, REPL, RAID, COMP, INTE, STRE, XREF, WORM, CODE, DWAL, DATA, FREE
...
127 region pointer slots x 32 bytes: RegionTypeId, Flags(ACTIVE/ENCRYPTED/COMPRESSED/WORM), StartBlock, BlockCount, UsedBlocks
...
80-279  200    ExtentCount(4) + 8 extents x 24 bytes
               Each: [StartBlock:8][BlockCount:4][Flags:4][LogicalOffset:8]
...
--- Reserved ---
496-511 16     SymLinkTarget / Reserved
```

**Key improvements over v1.0:**
- Extent-based: 8 extents can address terabytes (vs 12 pointers = 48 KiB max inline)
...
- Content hash inline: enables dedup detection and quick integrity check
...
Replaces flat checksum table. Leaf: 1 hash per data block. Internal: 128 hashes per block. Enables O(log N) subset verification, air-gap integrity proofs, tamper-proof chain anchoring. ~4x space vs flat table but dramatically superior capabilities.
...
| Integrity Tree (Merkle) | 8.1 GiB | 0.79% |
...
1. **Block trailers over separate checksum table** — 0.39% cost buys self-verifiable blocks, offline forensic recovery, torn-write detection
2. **Extent-based inodes over pointer arrays** — same evolution as ext3→ext4; 8 extents address terabytes
3. **Inline tag storage** — converts 2+ I/O to 1 I/O for 80% of tagged objects
4. **Dual WAL** — metadata and data WAL separated for reduced contention
5. **Merkle tree over flat checksums** — O(log N) subset proofs for air-gap verification
6. **WORM as physical region** — allocator-level immutability, not just a flag
7. **Region indirection** — pointer table enables online defrag, resize, feature toggling
8. **Streaming ring buffer** — zero fragmentation for append-only workloads
...
19-31 Reserved          —       —                             Future expansion                 —
```
...
INTG (Integrity) levels:
  0x0: Disabled (no checksums beyond Universal Block Trailer XxHash64)
  0x1: Basic — flat checksum table (8B per block, xxHash64)
  0x2: Standard — Merkle tree (BLAKE3, 32B per leaf)
  0x3: Enhanced — Merkle tree + AEAD authentication tags
  0xF: Maximum — Merkle tree + AEAD + blockchain anchoring + air-gap proof generation
...
function CalculateInodeSize(selectedModules: Module[]): InodeSpec {
    ...
    inode_size = ceil(raw_size / 64) * 64
...
Example 1: Minimal (no modules)
  ...
  Modules:            0 bytes
  Raw:              304 bytes
  Rounded to 64:    320 bytes (next multiple of 64)
  Padding:           16 bytes (available for future module addition)
  InodeSize = 320
