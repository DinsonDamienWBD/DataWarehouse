...
0x20    8B    SmaBitmapStart          uint64 LE — block offset of SMA's own allocation bitmap
0x28    8B    SmaRegionDirectory      uint64 LE — block offset of SMA's region directory
0x30    16B   SmaIntegritySeal        HMAC-BLAKE3 of SMA header
0x40    960B  Reserved                Zero-filled
```
...
### Sizing: Dynamic, Not Fixed

The SMA is NOT a fixed 50/50 partition. It grows on demand from the end of the VDE:

- **Initial allocation:** When SMA is first enabled, the engine reserves a small seed (default: 0.1% of VDE capacity or 1 GB, whichever is larger). The SMA Header and bitmap are written at the end of the VDE.
- **Growth:** When the SMA bitmap shows >80% utilization, the engine extends the SMA by doubling its size (up to a configurable maximum, default: 10% of VDE capacity). Extension is a background operation — update `SmaStartBlock` in superblock, extend SMA bitmap.
- **Shrink:** When SMA utilization drops below 20% for 7+ days, the Background Vacuum reclaims unused SMA blocks back to the primary free pool.
...
Typical SMA sizes by workload:
| Workload | SMA Size (% of VDE) | Contents |
|----------|---------------------|----------|
| Golden Path only | 0% (SMA disabled) | N/A |
| + AI classification | ~1-2% | Vector embeddings (~1KB per file) |
| + Full compliance | ~2-5% | Audit documents, PII scans |
| + Search indexing | ~3-8% | Inverted indexes, B+-trees |
| + Everything | ~5-10% | All extended metadata |

A 100 TB VDE with full SMA: ~90-95 TB primary data, ~5-10 TB extended metadata.

### Enabling the SMA

The SMA is controlled by `IncompatibleFeatureFlags` bit 9 (`INCOMPAT_HAS_SMA`):

- **Creation time:** `dw create --sma` enables the SMA. Default: OFF.
- **Post-creation:** `dw upgrade --enable-sma` adds the SMA to an existing VDE (online, non-destructive). The engine allocates SMA blocks from free space at the end of the VDE, writes the SMA Header, and sets the superblock flag.
- **Cannot be disabled once enabled** (data in the SMA would be orphaned). Can be emptied: `dw sma drain` migrates all SMA metadata to external sidecar (`.dwvd.meta`), then the SMA blocks are reclaimed.
