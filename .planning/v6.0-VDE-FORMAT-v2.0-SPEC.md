...
320     168    InlineTagArea            Up to 5 compact tags (168B ÷ 32B/tag = 5 full slots):
                                        Each: [NamespaceHash:4][NameHash:4][ValueType:1][ValueLen:1][Value:≤22B]
                                        Eliminates overflow I/O for ~85% of tagged objects.
                                        (Reduced from 176B to 168B to make room for SmaBlockPtr
                                        at 0x1E8. One partial tag slot traded for SMA linkage.
                                        Tag capacity: 5 full tags, unchanged from effective v2.0
                                        capacity since the 176B envelope only fit 5 complete
                                        32B tags with 16B remainder — the 8B reduction has no
                                        practical effect on inline tag count.)

── SmaBlockPtr (8 bytes, fixed-offset at 0x1E8) ──
488     8      SmaBlockPtr              uint64 LE — block address in the Secondary Metadata Area
                                        (SMA) where this inode's extended metadata linkage block
                                        begins. Value 0 = no SMA metadata for this inode (default,
                                        common case). Non-zero = SMA region is active. This field
                                        is NEVER read during normal read()/write() I/O; it is
                                        checked only for explicit metadata queries (dw vectors,
                                        dw lineage, dw compliance, etc.). See §SMA section.
...
- InlineTagArea expanded to 168 bytes (from 128B; originally allocated as 176B but 8B reallocated to SmaBlockPtr at 0x1E8) — fits 5 full enterprise tags inline, dramatically reducing TagIndexRegion I/O
- `SmaBlockPtr` (8B at 0x1E8): per-inode pointer to Secondary Metadata Area linkage block; zero when no extended metadata is active, never read during normal I/O
...
```
Inode offset 0x1E8:  SmaBlockPtr  uint64 LE  (fixed-offset, inode bytes 488-495)
```
...
**Layout note:** `SmaBlockPtr` occupies the 8 bytes immediately before the `DataOsModuleArea`
(0x1F0-0x1FF). It was placed here by shrinking `InlineTagArea` from 176B to 168B — no
effective tag-count loss since 176B ÷ 32B = 5 complete tags with 16B remainder, and
168B ÷ 32B = 5 complete tags with 8B remainder. The `DataOsModuleArea` boundary (0x1F0)
is **unchanged**; only the tag area was trimmed.
