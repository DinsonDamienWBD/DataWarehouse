...
TAGS (Tags) levels:
  0x0: Disabled
  0x1: Basic — InlineTagArea in inode only (168B, ~5 tags), no index region
...
**Recommendation:** Default to 1024B for any deployment that enables more than 4 modules. The "zero additional I/O" claim holds because NVMe pages are 4KB — four 1024B inodes per page vs eight 512B inodes per page, same number of NVMe reads for sequential inode scans.
...
Tags                 InlineTagArea (168B) in inode,              Tags stored in external inverted     Tags as key-value pairs
...
| 9 | INCOMPAT_HAS_SMA | Secondary Metadata Area is present |
...
46. **Data Classification Tags** — Already covered by inline tags (168B InlineTagArea at inode offset 320, within the 176B TAGS module starting at offset 312).
...
Stage 3: Tag Indexer
  - Write InlineTagArea (168B at inode offset 320)
...
Phase 6 — VDE Write Pipeline (SDK VirtualDiskEngine — all 124 format features)
  ...
  6.3 Inline tag area (168B, RoaringBitmap index update)
...
ext4's inode is 256 bytes with ~13 fields. DW's inode is 512 bytes with module areas, inline tags (168B), and extent tree pointers. The Metadata WAL journal entry is also larger because it tracks module state for crash recovery. This adds ~0.3ms for inode serialization and ~0.2ms for the larger WAL entry.
...
| G12 | Tags | 168B inline area in inode | Inode is 512B regardless — those bytes exist whether used or not. Zero marginal I/O for tag data. Tag INDEX updates are batched in background. | Rich metadata, search, classification. Self-documenting storage. |
...
- `SYNC`: SMA write is committed to NVMe BEFORE the primary write returns to the application. The write path becomes: primary I/O → SMA sync write → return to app. Adds ~0.1-0.5ms per sync SMA block.
...
- **Garbage Collection — Two-Phase Delete:** Deleting a file with SMA metadata requires freeing both primary extents and SMA blocks. The engine uses a two-phase approach: (1) Mark the inode as deleted in the primary area (journaled), setting `SmaBlockPtr` to a tombstone value (`0xFFFFFFFFFFFFFFFF`). (2) A background SMA GC thread scans for tombstoned `SmaBlockPtr` values and frees the corresponding SMA blocks. If a crash occurs between phases, the SMA blocks are leaked temporarily ("dark matter"). The SMA GC thread detects and reclaims orphaned SMA blocks on the next mount by cross-referencing SMA allocations against live inode `SmaBlockPtr` values. Worst case: orphaned SMA blocks persist until next mount. The SMA Header tracks `OrphanedBlocks: uint64` for monitoring.
