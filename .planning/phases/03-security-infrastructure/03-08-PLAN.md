...
must_haves:
  truths:
    ...
    - "D migration items complete (plugin references updated, migration guide, deprecation)"
    - "T95 B15, B16, C, D items in TODO.md are marked [x]"
    - "UltimateAccessControl plugin builds clean with ALL strategies"
  artifacts:
  ...
  key_links:
    ...
    - from: "Features"
      ...
      via: "Message bus for AI delegation"
      pattern: "_messageBus"
---
...
Purpose: This is the final plan for T95, completing the remaining strategy groups and ensuring the entire UltimateAccessControl plugin builds cleanly with 100+ strategies registered.
...
<task type="auto">
  <name>Task 2: Implement advanced features, migration, and final sync (T95.C, D)</name>
  <files>
...
2. Implement T95.C Advanced Features (C1-C10):

   **C1 MlAnomalyDetection** - Message bus to T90 for ML, fallback to statistical Z-score anomaly detection
   ...
   **C9 AiSecurityIntegration** - Message bus to T90 for AI-powered security decisions
   **C10 SiemConnector** - SIEM event forwarding (Splunk, Sentinel, ELK) via Syslog/REST

   AI-dependent features (C1, C9) MUST use message bus with rule-based fallbacks.

3. Implement T95.D Migration (D1-D5):

   **D1** - Verify all plugin references use UltimateAccessControl (scan for old AccessControl/IAM/Security plugin refs)
   **D2** - Create migration guide as XML doc comments in plugin (not standalone doc)
   **D3** - Add [Obsolete] attributes to any deprecated security plugins found
   **D4** - Mark for removal (defer actual deletion to Phase 18 per ROADMAP convention)
   **D5** - Update documentation via XML doc comments

4. Final build and full strategy count:
   - `dotnet build` the entire project
   - Count total IAccessControlStrategy implementations in the assembly
   - Verify auto-discovery registers all strategies

5. Update Metadata/TODO.md:
   - Mark T95.B15.1-B15.10 as [x] (lines ~8204-8213)
   - Mark T95.B16.1-B16.10 as [x] (lines ~8215-8224)
   - Mark T95.C1-C10 as [x] (lines ~8230-8239)
   - Mark T95.D1-D5 as [x] (lines ~8245-8249)
   - Update the Task 95 summary to show all phases complete

6. Verify T95 section has ZERO remaining [ ] items.
  </action>
  <verify>
`dotnet build` succeeds for full project.
Total strategy count exceeds 100.
TODO.md T95 section has zero remaining [ ] items.
Grep for `\[ \]` in T95 section returns empty.
  </verify>
  <done>
T95 is fully complete: 100+ strategies, 10 advanced features, migration done.
...
4. D1-D5 migration tasks completed
5. Total strategy count > 100
6. TODO.md T95 section: ZERO [ ] items remaining
7. Full solution `dotnet build` succeeds
</verification>
...
<output>
After completion, create `.planning/phases/03-security-infrastructure/03-08-SUMMARY.md`
</output>
