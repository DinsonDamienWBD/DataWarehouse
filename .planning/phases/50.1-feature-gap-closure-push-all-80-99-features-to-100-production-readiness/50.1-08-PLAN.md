...
<objective>
Add production readiness infrastructure to media domain strategies: H.264 10-bit, H.265 10-bit, Adaptive Bitrate Streaming, Video Watermarking, and Image Operations (resize/rotate/crop).

Purpose: Push Domain 04 (Media) features from 90% to 100%. These strategies handle video encoding, streaming, and image manipulation.

Output: 6+ strategy files updated with production infrastructure.
</objective>
...
1. H.264 10-bit Encoding (Transcoding plugin)
2. H.265 10-bit Encoding (Transcoding plugin)
3. Adaptive Bitrate Streaming x2 (Transcoding plugin — likely HLS + DASH)
4. Video Watermarking (Transcoding plugin)
5. Image Resizing (Transcoding plugin)
6. Image Rotation (Transcoding plugin)
7. Image Cropping (Transcoding plugin)

IMPORTANT: "10-bit encoding" may be a mode/flag within the H264/H265 strategy, not a separate class. Check the strategy to see how bit depth is configured.

"Video Watermarking" may be in a separate file or part of the video codec strategies. Find it:
```bash
find Plugins/DataWarehouse.Plugins.Transcoding.Media -name "*Watermark*" -o -name "*watermark*"
```

Image resize/rotate/crop may be methods within image strategies, not separate classes.
</context>
...
<task type="auto">
  ...
  <action>
FIRST: Locate the watermarking and image operation strategy files:
```bash
find Plugins/DataWarehouse.Plugins.Transcoding.Media -name "*.cs" | xargs grep -l "class.*Watermark\|Watermark.*Strategy\|ImageResize\|ImageCrop\|ImageRotat"
```

For Video Watermarking strategy:
...
For Image Operation strategies (resize, rotate, crop — may be in JpegImageStrategy, PngImageStrategy, or separate files):
...
4. **Counters**: `IncrementCounter("image.resize")`, `IncrementCounter("image.rotate")`, `IncrementCounter("image.crop")`.
...
`dotnet build DataWarehouse.slnx --no-restore 2>&1 | tail -5` — 0 errors 0 warnings.
  ...
  <done>Video watermarking and image operation strategies have full production infrastructure. Build passes. Domain 04 complete.</done>
</task>
...
<success_criteria>
H.264 10-bit, H.265 10-bit, ABR streaming, video watermarking, and image ops pushed to 100%. Zero build errors. Domain 04 complete.
</success_criteria>
