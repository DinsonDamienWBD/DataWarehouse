...
files_modified:
  ...
  - Plugins/DataWarehouse.Plugins.UltimateDeployment/Strategies/FeatureFlags/FeatureFlagStrategies.cs
  - Plugins/DataWarehouse.Plugins.UltimateDeployment/Strategies/DeploymentPatterns/
...
must_haves:
  truths:
    ...
    - "ABTesting strategy has config validation for traffic split, experiment ID"
    - "Shadow deployment strategy has config validation for shadow endpoint, traffic percentage"
...
Output: 5+ strategy files updated with production infrastructure.
</objective>
...
Domain 14 features at 90%: Stackdriver (UniversalObservability), Elasticsearch (UniversalObservability)
Domain 16 features at 90%: ABTesting (Unknown plugin), Shadow Deployment (Unknown plugin), Kubernetes Deployment (Unknown plugin)

For Domain 16 "Unknown" plugins — these are likely in UltimateDeployment:
- ABTesting → FeatureFlags/FeatureFlagStrategies.cs (or a dedicated ABTestingStrategy)
- Shadow Deployment → DeploymentPatterns/ (alongside BlueGreen, Canary, RollingUpdate)
- Kubernetes Deployment → ContainerOrchestration/KubernetesStrategies.cs

Locate:
```bash
find Plugins/DataWarehouse.Plugins.UltimateDeployment -name "*.cs" | xargs grep -l "ABTest\|ShadowDeploy\|Kubernetes"
```
</context>
...
<task type="auto">
  ...
  <action>
For Stackdriver metrics strategy:
...
For Elasticsearch logging strategy:
...
<task type="auto">
  ...
  <action>
FIRST: Locate the ABTesting and Shadow Deployment strategy classes:
```bash
find Plugins/DataWarehouse.Plugins.UltimateDeployment -name "*.cs" | xargs grep -l "class.*ABTest\|class.*ShadowDeploy\|class.*Shadow"
```

For ABTesting strategy:
...
For Shadow Deployment strategy:
...
For Kubernetes Deployment strategy:
