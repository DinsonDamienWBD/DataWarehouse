...
must_haves:
  truths:
    ...
    - "Directory.Build.props NoWarn line cleaned — zero suppressed diagnostics remain (CS0618/CS0649/CS0219/CS0414/CS0169/CS0162/CS0067/CS0168 all resolved)"
    - "Build with NO suppressions produces 0 warnings"
  artifacts:
...
**Step 5:** Remove ALL suppressed diagnostic codes from `Directory.Build.props`:
- Edit line 41: change `<NoWarn>$(NoWarn);CS0618;CS0649;CS0219;CS0414;CS0169;CS0162;CS0067;CS0168;CA1416;CA1418;CA2024;NU1608</NoWarn>` to `<NoWarn>$(NoWarn);NU1608</NoWarn>` (keep ONLY NU1608 for upstream NuGet dependency constraints which we can't fix)
- Keep `<TreatWarningsAsErrors>true</TreatWarningsAsErrors>` — this is the ultimate test
- Build: `dotnet build DataWarehouse.slnx --configuration Release --no-incremental`
- Every error means a diagnostic was NOT fixed in Wave 4 — fix it immediately
- Goal: build succeeds with 0 errors AND 0 warnings AND only NU1608 suppressed (external dependency issue)

**Step 6:** Run `dotnet build DataWarehouse.slnx -v diag 2>&1 | grep -E "warning|error" | grep -v NU1608 | grep -v NETSDK1057` to verify ZERO remaining diagnostics at any verbosity level.

**Step 7:** Run `dotnet build DataWarehouse.slnx -bl` for a final binary log. The binlog should show zero suppressions from our code.

**Step 6:** If any check fails, fix the issue immediately. Re-run until all checks pass.
  </action>
