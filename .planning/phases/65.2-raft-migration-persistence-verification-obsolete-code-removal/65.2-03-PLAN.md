...
must_haves:
  truths:
    ...
    - "UltimateConsensusPlugin(groupCount, votersPerGroup) constructor still works"
    - "UltimateConsensusTests.cs compiles and passes"
  ...
  artifacts:
    ...
    - path: "DataWarehouse.Tests/Consensus/UltimateConsensusTests.cs"
      provides: "Tests updated for rewired constructor"
      contains: "UltimateConsensusPlugin"
  key_links:
    ...
    - from: "Plugins/DataWarehouse.Plugins.UltimateConsensus/UltimateConsensusPlugin.cs"
      to: "DataWarehouse.SDK/Infrastructure/InMemory/InMemoryClusterMembership.cs"
      via: "uses SDK in-memory membership for local mode"
      pattern: "InMemoryClusterMembership"
---
...
<task type="auto">
  <name>Task 1: Rewire UltimateConsensusPlugin internals to SDK MultiRaftManager</name>
  <files>
    Plugins/DataWarehouse.Plugins.UltimateConsensus/UltimateConsensusPlugin.cs
    Plugins/DataWarehouse.Plugins.UltimateConsensus/RaftGroup.cs
    Plugins/DataWarehouse.Plugins.UltimateConsensus/LogEntry.cs
  ...
  <action>
**IMPORTANT CONTEXT:** UltimateConsensus is a plugin that references ONLY the SDK. The SDK provides `RaftConsensusEngine`, `MultiRaftManager`, `IRaftLogStore`, `FileRaftLogStore`, `InMemoryClusterMembership`, and `InMemoryP2PNetwork`. The plugin must use these SDK types for real distributed consensus.
...
**Step 2: Wire up SDK in-memory infrastructure for local mode:**
- The SDK already has production-ready single-node implementations:
  - `DataWarehouse.SDK.Infrastructure.InMemory.InMemoryClusterMembership` (implements `IClusterMembership`)
  - `DataWarehouse.SDK.Infrastructure.InMemory.InMemoryP2PNetwork` (implements `IP2PNetwork`)
- Use these directly -- do NOT create private inner classes or stubs.
- Instantiate `InMemoryClusterMembership` and `InMemoryP2PNetwork` as fields in the plugin.

**Step 3: Replace RaftGroup collection with MultiRaftManager:**
- Replace `BoundedDictionary<int, RaftGroup> _raftGroups` with SDK's `MultiRaftManager`
- The MultiRaftManager constructor takes: `IClusterMembership membership, IP2PNetwork network, RaftConfiguration? defaultConfig`
- Pass the `InMemoryClusterMembership` and `InMemoryP2PNetwork` instances from Step 2
...
- Use InMemoryRaftLogStore for single-node (local) mode

**Step 4: Adapt proposal routing:**
- Route proposals via MultiRaftManager.ProposeToGroupAsync instead of custom ConsistentHash + RaftGroup
- ConsistentHash can stay for group selection logic if MultiRaftManager doesn't handle it (check API)

**IMPORTANT — Preserve constructor API:**
- The existing constructor is `public UltimateConsensusPlugin(int groupCount = 3, int votersPerGroup = 3)`
- This MUST be preserved as-is. The constructor should internally create InMemoryClusterMembership and InMemoryP2PNetwork.
- Tests use `new UltimateConsensusPlugin(groupCount: 3, votersPerGroup: 3)` and `new UltimateConsensusPlugin()`.

**Step 5: Handle RaftGroup.cs and LogEntry.cs:**
...
- The LogEntry.cs in UltimateConsensus can be deleted -- use SDK's RaftLogEntry instead
...
- Do NOT remove Paxos/PBFT/ZAB strategies -- they are independent consensus implementations
  ...
  <verify>
    `dotnet build Plugins/DataWarehouse.Plugins.UltimateConsensus/DataWarehouse.Plugins.UltimateConsensus.csproj` compiles with zero errors. UltimateConsensusPlugin.cs contains references to `MultiRaftManager` and `InMemoryClusterMembership`.
  </verify>
  <done>
    UltimateConsensus delegates Raft consensus to SDK's RaftConsensusEngine via MultiRaftManager using InMemoryClusterMembership and InMemoryP2PNetwork for local mode. RaftGroup.cs removed or gutted. LogEntry.cs deleted (using SDK RaftLogEntry).
  </done>
</task>

<task type="auto">
  <name>Task 2: Update ConsensusPluginBase overrides, public API, and tests</name>
  <files>
    Plugins/DataWarehouse.Plugins.UltimateConsensus/UltimateConsensusPlugin.cs
    DataWarehouse.Tests/Consensus/UltimateConsensusTests.cs
  </files>
  <action>
**Step 1: Update ConsensusPluginBase overrides:**
...
**Step 2: Keep IRaftStrategy interface:**
...
**Step 3: Preserve all existing public API:**
...
**Step 5: Update UltimateConsensusTests.cs:**
- The test file at `DataWarehouse.Tests/Consensus/UltimateConsensusTests.cs` uses `new UltimateConsensusPlugin(groupCount: N, votersPerGroup: M)` in 8 places and parameterless `new UltimateConsensusPlugin()` in 1 place.
- Ensure the tests compile against the rewired plugin.
- If the tests reference `RaftGroup` or `LogEntry` types from the plugin, update to use SDK equivalents.
- The constructor signature `(int groupCount = 3, int votersPerGroup = 3)` must be preserved so tests don't break.
- If any test assertions reference removed internals (like checking internal RaftGroup state), update them to verify through the public API instead.

**Step 6: Build and run tests:**
```bash
dotnet build DataWarehouse.slnx
dotnet test DataWarehouse.Tests/DataWarehouse.Tests.csproj --filter "FullyQualifiedName~UltimateConsensus"
```
  </action>
  <verify>
    `dotnet build DataWarehouse.slnx` compiles with zero errors and zero warnings.
    `dotnet test DataWarehouse.Tests/DataWarehouse.Tests.csproj --filter "FullyQualifiedName~UltimateConsensus" --no-build` passes with zero failures.
  </verify>
  <done>
    All ConsensusPluginBase overrides route through MultiRaftManager. All public API preserved. UltimateConsensusTests.cs compiles and passes. Full solution builds clean.
  </done>
</task>
...
- `dotnet test DataWarehouse.Tests/DataWarehouse.Tests.csproj --filter "FullyQualifiedName~UltimateConsensus"` passes
- UltimateConsensusPlugin.cs references RaftConsensusEngine or MultiRaftManager
- UltimateConsensusPlugin.cs references InMemoryClusterMembership and InMemoryP2PNetwork
...
- Constructor `UltimateConsensusPlugin(int groupCount = 3, int votersPerGroup = 3)` preserved
</verification>
...
- Two-parameter constructor preserved for test compatibility
- UltimateConsensusTests.cs passes
