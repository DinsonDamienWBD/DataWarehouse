...
<task type="auto">
  ...
  <action>
0. **Pre-check:** Verify `Metadata/in-memory-analysis-report.json` exists. If it does not exist, STOP and report: "Missing prerequisite: Metadata/in-memory-analysis-report.json. Run `python Metadata/in-memory-analysis.py` first." Do NOT proceed without the analysis report.
