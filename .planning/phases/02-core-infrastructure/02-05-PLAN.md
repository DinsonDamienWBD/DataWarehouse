...
must_haves:
  truths:
    - "Plugin orchestrator accepts RAID operations via message bus and routes them to the correct strategy"
    - "Health monitoring reports drive health scores derived from real SMART data collection"
    - "Self-healing automatically detects degraded arrays and initiates rebuild using hot spares"
    - "Scrubbing engine verifies data integrity in background without blocking I/O operations"
  artifacts:
