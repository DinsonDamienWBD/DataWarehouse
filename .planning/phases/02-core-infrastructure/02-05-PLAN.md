...
must_haves:
  truths:
    - "AI optimization analyzes workloads and recommends RAID levels"
    - "Tiering supports SSD caching, NVMe tier, and auto-tiering"
    - "Performance features include parallel parity and SIMD optimization"
    - "AI features degrade gracefully when Intelligence plugin unavailable"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    **T91.G Performance**:
    ...
    CRITICAL: All AI features MUST check `IsIntelligenceAvailable` before using AI. If unavailable, fall back to rule-based/threshold-based logic. No hard dependency on Intelligence plugin.
  ...
  <done>
    AI optimization, tiering, and performance features all verified production-ready with proper Intelligence fallbacks.
  </done>
</task>
...
<verification>
- AI features degrade gracefully (check IsIntelligenceAvailable)
...
<success_criteria>
AI optimization, tiering, and performance features are production-ready with proper graceful degradation when Intelligence plugin is unavailable. SIMD parity uses real hardware acceleration.
</success_criteria>
