...
must_haves:
  truths:
    - "Context mixing strategies (ZPAQ, PAQ, CMIX, NNZ, PPM, PPMd) compress data using real prediction and arithmetic coding -- not simulation"
    - "Entropy coding strategies produce correct output: Huffman builds real frequency trees, Arithmetic subdivides real intervals, RLE encodes real runs"
    - "Differential strategies (Delta, Xdelta, Bsdiff, VCDIFF, Zdelta) compute real binary diffs that can reconstruct original data"
    - "Media-specific strategies use domain-appropriate techniques (FLAC uses LPC, TimeSeries uses delta-of-delta, DNA uses base encoding)"
  artifacts:
