...
type: execute
wave: 3
depends_on: ["06-02", "06-03", "06-04", "06-05", "06-06", "06-07", "06-08", "06-10", "06-11", "06-12"]
files_modified:
...
must_haves:
  truths:
    - "Requests arriving in one protocol can be translated to another protocol seamlessly via ProtocolBridgeService"
    - "All API requests are authenticated through UnifiedAuthService before reaching strategy handlers"
    - "API usage metrics (request count, latency percentiles, error rates) are tracked and publishable to observability"
    - "All 68+ strategies are discovered and registered by the orchestrator in a clean build"
  artifacts:
  ...
  key_links:
    - from: "Services/*.cs"
      ...
      pattern: "Service"
    - from: "Services/UnifiedAuthService.cs"
