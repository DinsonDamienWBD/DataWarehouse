...
<task type="auto">
  ...
  **Group C: Metadata/Adapter (update namespace)**
  ...
  6. `Metadata/Adapter/OperatingMode.cs`:
     ...
     - Replace the entire file content to just re-export SDK types using global usings or type aliases. The simplest approach: replace the file with type aliases that forward to SDK types:
     ```csharp
     // Re-export SDK Hosting types under the DataWarehouse.Integration namespace
     // for backward compatibility with linked compile includes.
     using DataWarehouse.SDK.Hosting;

     namespace DataWarehouse.Integration;

     // These type aliases provide backward compatibility.
     // The canonical types live in DataWarehouse.SDK.Hosting.
     // Consumers should migrate to using DataWarehouse.SDK.Hosting directly.
     ```
     HOWEVER, since the CLI links these files and the CLI Commands that used `DataWarehouse.Integration` namespace are dead code (excluded by `<Compile Remove="Commands\**" />`), the simplest safe approach is to replace the entire file content with just the SDK using directive and empty namespace. Actually, the CLEANEST approach: replace the type definitions in this file with `using` type aliases:
     ```csharp
     global using OperatingMode = DataWarehouse.SDK.Hosting.OperatingMode;
     global using ConnectionType = DataWarehouse.SDK.Hosting.ConnectionType;
     global using ConnectionTarget = DataWarehouse.SDK.Hosting.ConnectionTarget;
     global using InstallConfiguration = DataWarehouse.SDK.Hosting.InstallConfiguration;
     global using EmbeddedConfiguration = DataWarehouse.SDK.Hosting.EmbeddedConfiguration;
     ```
     Wait -- global using aliases would affect the entire CLI project. That could cause issues. Instead, just make the Metadata/Adapter files reference SDK directly. Change the namespace from `DataWarehouse.Integration` to use SDK types:
     Simply DELETE the type definitions from the file and add `using DataWarehouse.SDK.Hosting;` at the top. Since the namespace `DataWarehouse.Integration` will then be empty of types, the linked files won't cause issues. The CLI doesn't use these types anywhere in compiled code.
