...
namespace DataWarehouse.Plugins.UltimateCompression.Strategies.Emerging
{
    ...
    public sealed class LizardStrategy : CompressionStrategyBase
    {
        ...
        public async Task<StrategyHealthCheckResult> CheckHealthAsync(CancellationToken cancellationToken = default)
        {
            return await GetCachedHealthAsync(async ct =>
            {
                try
                {
                    ...
                    return new StrategyHealthCheckResult(
                        true,
                        "Lizard strategy healthy",
                        new Dictionary<string, object>
                        {
                            ["CompressOperations"] = GetCounter("lizard.compress"),
                            ["DecompressOperations"] = GetCounter("lizard.decompress")
                        });
                }
                catch (Exception ex)
                {
        ...
        {
            IncrementCounter("lizard.compress");

            if (input == null || input.Length == 0)
                return input ?? Array.Empty<byte>();

            if (input.Length > MaxInputSize)
                throw new ArgumentException($"Input exceeds maximum size of {MaxInputSize / (1024 * 1024)} MB for Lizard");
        ...
        {
            IncrementCounter("lizard.decompress");
