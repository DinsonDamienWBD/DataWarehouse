...
{
    /// <summary>
    /// Vultr Object Storage strategy using S3-compatible API.
    ...
    /// - Competitive pricing ($5/1TB storage)
    /// - Integrated with Vultr Cloud Compute
    /// - Presigned URLs for temporary access
    /// - CORS configuration for web applications
    /// - Bucket and object ACLs for access control
    /// - Lifecycle policies for automatic expiration
    /// - Automatic retry with exponential backoff
    /// - Low latency with global edge network
    /// </summary>
    /// <remarks>
    /// Vultr Object Storage provides S3-compatible object storage with global availability.
    /// Optimized for storing static content, backups, media files, application data, and CDN origins.
    ///
    /// Endpoint Format: {region}.vultrobjects.com
    /// Supported Regions:
    /// - ewr1: New Jersey, USA (East Coast)
    /// - ord1: Chicago, USA (Central)
    /// - dfw1: Dallas, USA (South Central)
    /// - sea1: Seattle, USA (West Coast)
    /// - lax1: Los Angeles, USA (West Coast)
    /// - sjc1: Silicon Valley, USA (West Coast)
    /// - atl1: Atlanta, USA (Southeast)
    /// - ams1: Amsterdam, Netherlands (Europe)
    /// - fra1: Frankfurt, Germany (Europe)
    /// - lhr1: London, UK (Europe)
    /// - mad1: Madrid, Spain (Europe)
    /// - cdg1: Paris, France (Europe)
    /// - waw1: Warsaw, Poland (Europe)
    /// - sgp1: Singapore (Asia Pacific)
    /// - nrt1: Tokyo, Japan (Asia Pacific)
    /// - icn1: Seoul, South Korea (Asia Pacific)
    /// - syd1: Sydney, Australia (Asia Pacific)
    /// - yto1: Toronto, Canada (North America)
    /// - mex1: Mexico City, Mexico (North America)
    /// - bog1: Bogotá, Colombia (South America)
    /// - sao1: São Paulo, Brazil (South America)
    /// - jnb1: Johannesburg, South Africa (Africa)
    /// - del1: Delhi, India (Asia)
    /// - blr1: Bangalore, India (Asia)
    /// - bom1: Mumbai, India (Asia)
    ///
    /// Configuration Requirements:
    /// - Region: Vultr Object Storage region identifier (e.g., "ewr1")
    /// - Bucket: Bucket name (globally unique)
    /// - AccessKey: Vultr Object Storage Access Key
    /// - SecretKey: Vultr Object Storage Secret Key
    ///
    /// Features:
    ...
    /// - Low latency through edge network
    ///
    /// Pricing Model:
    /// - Storage: $5/month for 1TB (most competitive)
    /// - Additional storage: $0.005/GB/month
    /// - Transfer: First 1TB free, then $0.01/GB
    /// - No API request charges
    /// - No ingress fees
    /// - No minimum storage duration
    ///
    /// Integration Benefits:
    /// - Seamless integration with Vultr Compute Instances
    /// - Optimized transfer within Vultr network
    /// - Managed through Vultr Cloud Portal
    /// - Unified billing with other Vultr services
    ...
    public class VultrObjectStorageStrategy : UltimateStorageStrategyBase
    {
        ...
        public override StorageTier Tier => StorageTier.Hot; // Vultr Object Storage is hot storage
        ...
        public override StorageCapabilities Capabilities => new StorageCapabilities
        {
            ...
            SupportsVersioning = true, // Vultr supports versioning
        ...
        /// <summary>
        /// Initializes the Vultr Object Storage strategy.
        /// </summary>
        protected override Task InitializeCoreAsync(CancellationToken ct)
        {
            // Load required configuration
            _region = GetConfiguration<string>("Region")
                ?? throw new InvalidOperationException("Vultr region is required. Set 'Region' (e.g., 'ewr1', 'lax1', 'ams1', 'sgp1').");
            ...
            _accessKey = GetConfiguration<string>("AccessKey")
                ?? throw new InvalidOperationException("Vultr Access Key is required. Set 'AccessKey' in configuration.");

            _secretKey = GetConfiguration<string>("SecretKey")
                ?? throw new InvalidOperationException("Vultr Secret Key is required. Set 'SecretKey' in configuration.");

            // Validate region
            var validRegions = new[] {
                "ewr1", "ord1", "dfw1", "sea1", "lax1", "sjc1", "atl1",
                "ams1", "fra1", "lhr1", "mad1", "cdg1", "waw1",
                "sgp1", "nrt1", "icn1", "syd1",
                "yto1", "mex1", "bog1", "sao1",
                "jnb1", "del1", "blr1", "bom1"
            };
            if (!validRegions.Contains(_region.ToLowerInvariant()))
            {
                throw new InvalidOperationException(
                    $"Invalid Vultr region '{_region}'. Valid regions: {string.Join(", ", validRegions)}");
            }

            // Build endpoint
            _endpoint = GetConfiguration<string>("Endpoint", $"{_region}.vultrobjects.com");
            ...
            {
                throw new InvalidOperationException("Vultr Object Storage multipart part size must be at least 5MB");
            }

            if (_multipartPartSizeBytes > 5L * 1024 * 1024 * 1024)
            {
                throw new InvalidOperationException("Vultr Object Storage multipart part size cannot exceed 5GB");
            }

            // Create AWS S3 client configured for Vultr Object Storage
            ...
            CancellationToken ct)
        {
            ...
            catch (Exception)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"[VultrObjectStorageStrategy.StoreMultipartAsync] {ex.GetType().Name}: {ex.Message}");
        ...
        protected override async Task DeleteAsyncCore(string key, CancellationToken ct)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[VultrObjectStorageStrategy.DeleteAsyncCore] {ex.GetType().Name}: {ex.Message}");
                // Ignore errors
            }
        ...
        protected override async Task<StorageHealthInfo> GetHealthAsyncCore(CancellationToken ct)
        {
            try
            {
                ...
                return new StorageHealthInfo
                {
                    ...
                    Message = $"Vultr Object Storage bucket '{_bucket}' is accessible in region {_region}",
            ...
            catch (Exception ex)
            {
                return new StorageHealthInfo
                {
                    ...
                    Message = $"Failed to access Vultr Object Storage bucket '{_bucket}' in region {_region}: {ex.Message}",
                    CheckedAt = DateTime.UtcNow
                };
            }
        }
        ...
        {
            // Vultr Object Storage has no practical capacity limit from client perspective
            return Task.FromResult<long?>(null);
        }

        #endregion

        #region Vultr-Specific Operations
        ...
        /// <summary>
        /// Gets the versioning status for the bucket.
        /// </summary>
        /// <param name="ct">Cancellation token.</param>
        ...
        public async Task<VersionStatus?> GetVersioningStatusAsync(CancellationToken ct = default)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[VultrObjectStorageStrategy.GetVersioningStatusAsync] {ex.GetType().Name}: {ex.Message}");
                return null;
            }
        }
        ...
        protected override int GetMaxKeyLength() => 1024; // Vultr max object name length (S3-compatible)
