...
{
    /// <summary>
    /// Scaleway Object Storage strategy using S3-compatible API.
    ...
    /// - Multi-region support across Europe (Paris, Amsterdam, Warsaw)
    ...
    /// - Integrated with Scaleway Cloud Platform
    ...
    /// </summary>
    /// <remarks>
    /// Scaleway Object Storage provides S3-compatible object storage with European presence.
    /// Optimized for storing static content, backups, media files, application data, and archives.
    ///
    /// Endpoint Format: s3.{region}.scw.cloud
    /// Supported Regions:
    /// - fr-par: Paris, France (Primary EU region)
    /// - nl-ams: Amsterdam, Netherlands (EU region)
    /// - pl-waw: Warsaw, Poland (EU region)
    ///
    /// Configuration Requirements:
    /// - Region: Scaleway Object Storage region identifier (e.g., "fr-par")
    /// - Bucket: Bucket name (globally unique within Scaleway)
    /// - AccessKey: Scaleway Access Key (SCW...)
    ...
    ///   * No retrieval fees
    ///
    /// - ONEZONE_IA: Infrequent access storage (single AZ)
    ...
    /// - Low latency within European regions
    ///
    /// Pricing Model (as of 2024):
    /// - Storage STANDARD: €0.01/GB/month
    /// - Storage ONEZONE_IA: €0.006/GB/month
    /// - Storage GLACIER: €0.002/GB/month
    /// - Transfer: €0.01/GB egress (first 75GB free)
    /// - No API request charges
    /// - No ingress fees
    /// - No minimum storage duration for STANDARD
    ///
    /// Integration Benefits:
    /// - Seamless integration with Scaleway Instances
    /// - Optimized transfer within Scaleway network
    /// - Managed through Scaleway Console
    /// - Unified billing with other Scaleway services
    ...
    public class ScalewayObjectStorageStrategy : UltimateStorageStrategyBase
    {
        ...
        private string _region = "fr-par";
        private string _bucket = string.Empty;
        private string _accessKey = string.Empty;
        private string _secretKey = string.Empty;
        private string _storageClass = "STANDARD"; // STANDARD, ONEZONE_IA, GLACIER
        ...
        public override StorageCapabilities Capabilities => new StorageCapabilities
        {
            ...
            SupportsVersioning = true, // Scaleway supports versioning
            SupportsTiering = true, // STANDARD, ONEZONE_IA, GLACIER
        ...
        /// <summary>
        /// Initializes the Scaleway Object Storage strategy.
        /// </summary>
        protected override Task InitializeCoreAsync(CancellationToken ct)
        {
            // Load required configuration
            _region = GetConfiguration<string>("Region")
                ?? throw new InvalidOperationException("Scaleway region is required. Set 'Region' (e.g., 'fr-par', 'nl-ams', 'pl-waw').");
            ...
            _accessKey = GetConfiguration<string>("AccessKey")
                ?? throw new InvalidOperationException("Scaleway Access Key is required. Set 'AccessKey' in configuration.");

            _secretKey = GetConfiguration<string>("SecretKey")
                ?? throw new InvalidOperationException("Scaleway Secret Key is required. Set 'SecretKey' in configuration.");

            // Validate region
            var validRegions = new[] { "fr-par", "nl-ams", "pl-waw" };
            if (!validRegions.Contains(_region.ToLowerInvariant()))
            {
                throw new InvalidOperationException(
                    $"Invalid Scaleway region '{_region}'. Valid regions: {string.Join(", ", validRegions)}");
            }

            // Build endpoint
            _endpoint = GetConfiguration<string>("Endpoint", $"s3.{_region}.scw.cloud");

            // Load optional configuration
            _storageClass = GetConfiguration("StorageClass", "STANDARD").ToUpperInvariant();

            // Validate storage class
            var validStorageClasses = new[] { "STANDARD", "ONEZONE_IA", "GLACIER" };
            if (!validStorageClasses.Contains(_storageClass))
            {
                throw new InvalidOperationException(
                    $"Invalid Scaleway storage class '{_storageClass}'. Valid classes: {string.Join(", ", validStorageClasses)}");
            }
            ...
            {
                throw new InvalidOperationException("Scaleway Object Storage multipart part size must be at least 5MB");
            }

            if (_multipartPartSizeBytes > 5L * 1024 * 1024 * 1024)
            {
                throw new InvalidOperationException("Scaleway Object Storage multipart part size cannot exceed 5GB");
            }

            // Create AWS S3 client configured for Scaleway Object Storage
            var s3Config = new AmazonS3Config
            {
                ServiceURL = $"https://{_endpoint}",
                AuthenticationRegion = _region,
                ForcePathStyle = false, // Scaleway uses virtual-hosted-style URLs
        ...
        protected override async Task<StorageHealthInfo> GetHealthAsyncCore(CancellationToken ct)
        {
            try
            {
                ...
                return new StorageHealthInfo
                {
                    ...
                    Message = $"Scaleway Object Storage bucket '{_bucket}' is accessible in region {_region} with storage class {_storageClass}",
            ...
            catch (Exception ex)
            {
                return new StorageHealthInfo
                {
                    ...
                    Message = $"Failed to access Scaleway Object Storage bucket '{_bucket}' in region {_region}: {ex.Message}",
                    CheckedAt = DateTime.UtcNow
                };
            }
        }
        ...
        {
            // Scaleway Object Storage has no practical capacity limit from client perspective
            return Task.FromResult<long?>(null);
        }

        #endregion

        #region Scaleway-Specific Operations
        ...
        /// Allows moving data between STANDARD, ONEZONE_IA, and GLACIER tiers.
        /// </summary>
        /// <param name="key">Object key.</param>
        /// <param name="targetStorageClass">Target storage class (STANDARD, ONEZONE_IA, GLACIER).</param>
        /// <param name="ct">Cancellation token.</param>
        ...
        /// <summary>
        /// Maps Scaleway storage class string to S3StorageClass enum.
        /// </summary>
        ...
        protected override int GetMaxKeyLength() => 1024; // Scaleway max object name length (S3-compatible)
