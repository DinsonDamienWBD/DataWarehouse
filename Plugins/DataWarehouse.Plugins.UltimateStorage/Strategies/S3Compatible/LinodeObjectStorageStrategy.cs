...
{
    /// <summary>
    /// Linode Object Storage strategy using S3-compatible API.
    /// Full production features include:
    /// - S3-compatible API via AWS SDK (100% S3 API compatible)
    /// - Global availability with multiple cluster locations
    /// - High durability (99.999999999% - 11 nines)
    /// - Server-side encryption (AES-256)
    /// - Large file support with multipart uploads
    /// - Integrated with Linode infrastructure
    /// - Cost-effective pricing ($5/250GB)
    /// - No egress fees within Linode network
    ...
    /// </summary>
    /// <remarks>
    /// Linode Object Storage provides S3-compatible object storage integrated with Linode Cloud.
    /// Optimized for storing static content, backups, media files, and application data.
    ///
    /// Endpoint Format: {cluster}.linodeobjects.com
    /// Supported Clusters:
    /// - us-east-1: Newark, NJ, USA (East Coast)
    /// - us-southeast-1: Atlanta, GA, USA (Southeast)
    /// - eu-central-1: Frankfurt, Germany (Europe)
    /// - ap-south-1: Singapore (Asia Pacific)
    ///
    /// Configuration Requirements:
    /// - Cluster: Linode Object Storage cluster (e.g., "us-east-1")
    /// - Bucket: Bucket name
    /// - AccessKey: Linode Object Storage Access Key
    /// - SecretKey: Linode Object Storage Secret Key
    ///
    /// Features:
    ...
    ///
    /// Pricing Model:
    /// - Storage: $5/month for 250GB, $0.02/GB beyond
    /// - Transfer: $0.005/GB outbound (to internet)
    /// - Free transfer within Linode network
    /// - No API request charges
    /// - No ingress fees
    ///
    /// Integration Benefits:
    /// - Seamless integration with Linode Compute Instances
    /// - Free transfer between Object Storage and Linode services
    /// - Managed through Linode Cloud Manager
    /// - Unified billing with other Linode services
    /// </remarks>
    public class LinodeObjectStorageStrategy : UltimateStorageStrategyBase
    {
        private AmazonS3Client? _s3Client;
        private string _endpoint = string.Empty;
        private string _cluster = "us-east-1";
        ...
        private bool _enableServerSideEncryption = true; // Always recommended for Linode
        ...
        /// <summary>Gets the configured EnableCors value.</summary>
        internal bool EnableCors => _enableCors;

        public override string StrategyId => "linode-object-storage";
        public override string Name => "Linode Object Storage";
        public override StorageTier Tier => StorageTier.Hot; // Linode Object Storage is hot storage
        ...
        public override StorageCapabilities Capabilities => new StorageCapabilities
        {
            ...
            SupportsVersioning = false, // Linode doesn't support versioning currently
        ...
        /// <summary>
        /// Initializes the Linode Object Storage strategy.
        /// </summary>
        protected override Task InitializeCoreAsync(CancellationToken ct)
        {
            // Load required configuration
            _cluster = GetConfiguration<string>("Cluster")
                ?? throw new InvalidOperationException("Linode cluster is required. Set 'Cluster' (e.g., 'us-east-1', 'eu-central-1', 'ap-south-1', 'us-southeast-1').");
            ...
            _accessKey = GetConfiguration<string>("AccessKey")
                ?? throw new InvalidOperationException("Linode Access Key is required. Set 'AccessKey' in configuration.");

            _secretKey = GetConfiguration<string>("SecretKey")
                ?? throw new InvalidOperationException("Linode Secret Key is required. Set 'SecretKey' in configuration.");

            // Validate cluster
            var validClusters = new[] { "us-east-1", "us-southeast-1", "eu-central-1", "ap-south-1" };
            if (!validClusters.Contains(_cluster.ToLowerInvariant()))
            {
                throw new InvalidOperationException(
                    $"Invalid Linode cluster '{_cluster}'. Valid clusters: {string.Join(", ", validClusters)}");
            }

            // Build endpoint
            _endpoint = GetConfiguration<string>("Endpoint", $"{_cluster}.linodeobjects.com");
            ...
            {
                throw new InvalidOperationException("Linode Object Storage multipart part size must be at least 5MB");
            }

            if (_multipartPartSizeBytes > 5L * 1024 * 1024 * 1024)
            {
                throw new InvalidOperationException("Linode Object Storage multipart part size cannot exceed 5GB");
            }

            // Create AWS S3 client configured for Linode Object Storage
            var s3Config = new AmazonS3Config
            {
                ServiceURL = $"https://{_endpoint}",
                AuthenticationRegion = _cluster,
        ...
        protected override async Task<StorageHealthInfo> GetHealthAsyncCore(CancellationToken ct)
        {
            try
            {
                ...
                return new StorageHealthInfo
                {
                    ...
                    Message = $"Linode Object Storage bucket '{_bucket}' is accessible in cluster {_cluster}",
            ...
            catch (Exception ex)
            {
                return new StorageHealthInfo
                {
                    ...
                    Message = $"Failed to access Linode Object Storage bucket '{_bucket}' in cluster {_cluster}: {ex.Message}",
                    CheckedAt = DateTime.UtcNow
                };
            }
        }
        ...
        {
            // Linode Object Storage has no practical capacity limit from client perspective
            return Task.FromResult<long?>(null);
        }

        #endregion

        #region Linode-Specific Operations
        ...
        /// <summary>
        /// Sets the ACL (Access Control List) for a bucket.
        /// Supports making buckets public or private.
        /// </summary>
        /// <param name="acl">ACL to apply (e.g., Public, Private).</param>
        /// <param name="ct">Cancellation token.</param>
        public async Task SetBucketAclAsync(
            S3CannedACL acl,
            CancellationToken ct = default)
        {
            EnsureInitialized();
            ...
            await ExecuteWithRetryAsync(async () =>
                await _s3Client!.PutBucketAclAsync(request, ct), ct);
        }
        ...
        protected override int GetMaxKeyLength() => 1024; // Linode max object name length (S3-compatible)
