...
/// <summary>
/// XFS filesystem detection strategy.
/// </summary>
public sealed class XfsStrategy : FilesystemStrategyBase
{
    public override string StrategyId => "detect-xfs";
    public override string DisplayName => "XFS Filesystem";
    ...
    public override string SemanticDescription =>
        "XFS high-performance 64-bit journaling filesystem optimized for parallel I/O, " +
        "ideal for large files and streaming workloads.";
    public override string[] Tags => ["xfs", "linux", "journaling", "parallel", "large-files"];

    public override Task<FilesystemMetadata?> DetectAsync(string path, CancellationToken ct = default) =>
        Task.FromResult<FilesystemMetadata?>(new FilesystemMetadata
        {
            FilesystemType = "xfs",
            BlockSize = 4096,
            SupportsSparse = true,
    ...
    public override Task<FilesystemMetadata> GetMetadataAsync(string path, CancellationToken ct = default) =>
        Task.FromResult(new FilesystemMetadata { FilesystemType = "xfs" });
}

/// <summary>
/// ReFS filesystem detection strategy.
/// </summary>
public sealed class RefsStrategy : FilesystemStrategyBase
{
    public override string StrategyId => "detect-refs";
    public override string DisplayName => "ReFS Filesystem";
    public override FilesystemStrategyCategory Category => FilesystemStrategyCategory.Detection;
    public override FilesystemStrategyCapabilities Capabilities => new()
    {
        SupportsDirectIo = true, SupportsAsyncIo = true, SupportsMmap = true,
    ...
    public override string SemanticDescription =>
        "Windows Resilient File System (ReFS) with integrity streams, block cloning, " +
        "and automatic corruption repair for enterprise workloads.";
    public override string[] Tags => ["refs", "windows", "resilient", "integrity", "enterprise"];
    ...
    public override Task<FilesystemMetadata?> DetectAsync(string path, CancellationToken ct = default)
    {
        try
        {
            ...
            return Task.FromResult<FilesystemMetadata?>(new FilesystemMetadata
            {
                ...
                BlockSize = 64 * 1024, // ReFS uses 64KB clusters
                SupportsSparse = true,
    ...
    public override Task<FilesystemMetadata> GetMetadataAsync(string path, CancellationToken ct = default)
    {
        ...
        return Task.FromResult(result ?? new FilesystemMetadata { FilesystemType = "ReFS" });
    }
}
