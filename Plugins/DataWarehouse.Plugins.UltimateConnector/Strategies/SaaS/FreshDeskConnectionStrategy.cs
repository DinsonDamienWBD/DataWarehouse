...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.SaaS
{
    public class FreshDeskConnectionStrategy : SaaSConnectionStrategyBase
    {
        ...
        protected override async Task<bool> TestCoreAsync(IConnectionHandle handle, CancellationToken ct) { try { var response = await handle.GetConnection<HttpClient>().GetAsync("/api/v2/tickets?per_page=1", ct); return response.IsSuccessStatusCode; } catch (OperationCanceledException) { throw; } catch { return false; } }
