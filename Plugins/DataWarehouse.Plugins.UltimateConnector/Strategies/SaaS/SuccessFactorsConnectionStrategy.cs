...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.SaaS
{
    public class SuccessFactorsConnectionStrategy : SaaSConnectionStrategyBase
    {
        ...
        protected override async Task<bool> TestCoreAsync(IConnectionHandle handle, CancellationToken ct) { try { var response = await handle.GetConnection<HttpClient>().GetAsync("/odata/v2/User?$top=1", ct); return response.IsSuccessStatusCode; } catch (OperationCanceledException) { throw; } catch { return false; } }
