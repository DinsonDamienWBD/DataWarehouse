...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.SaaS
{
    public class SuccessFactorsConnectionStrategy : SaaSConnectionStrategyBase
    {
        ...
        protected override async Task<IConnectionHandle> ConnectCoreAsync(ConnectionConfig config, CancellationToken ct) { var datacenter = GetConfiguration<string>(config, "Datacenter", null!); var endpoint = $"https://api{datacenter}.successfactors.com"; var httpClient = new HttpClient { BaseAddress = new Uri(endpoint), Timeout = config.Timeout }; return new DefaultConnectionHandle(httpClient, new Dictionary<string, object> { ["Datacenter"] = datacenter, ["Endpoint"] = endpoint }); }
