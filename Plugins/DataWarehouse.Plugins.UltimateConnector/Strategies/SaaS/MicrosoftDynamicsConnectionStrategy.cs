...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.SaaS
{
    public class MicrosoftDynamicsConnectionStrategy : SaaSConnectionStrategyBase
    {
        ...
        protected override async Task<IConnectionHandle> ConnectCoreAsync(ConnectionConfig config, CancellationToken ct) { var org = GetConfiguration<string>(config, "Organization", null!); var region = GetConfiguration(config, "Region", "crm"); var endpoint = $"https://{org}.{region}.dynamics.com"; var httpClient = new HttpClient { BaseAddress = new Uri(endpoint), Timeout = config.Timeout }; return new DefaultConnectionHandle(httpClient, new Dictionary<string, object> { ["Organization"] = org, ["Endpoint"] = endpoint }); }
