...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.SaaS
{
    public class SlackConnectionStrategy : SaaSConnectionStrategyBase
    {
        ...
        public override string SemanticDescription => "Connects to Slack collaboration platform using HTTPS REST API.";
        public override string[] Tags => new[] { "slack", "collaboration", "messaging", "saas", "rest-api" };
        public SlackConnectionStrategy(ILogger? logger = null) : base(logger) { }
        protected override async Task<IConnectionHandle> ConnectCoreAsync(ConnectionConfig config, CancellationToken ct) { var httpClient = new HttpClient { BaseAddress = new Uri("https://slack.com/api"), Timeout = config.Timeout }; return new DefaultConnectionHandle(httpClient, new Dictionary<string, object> { ["Endpoint"] = "https://slack.com/api" }); }
        protected override async Task<bool> TestCoreAsync(IConnectionHandle handle, CancellationToken ct) { try { var response = await handle.GetConnection<HttpClient>().GetAsync("/api.test", ct); return response.IsSuccessStatusCode; } catch { return false; } }
        ...
        protected override async Task<ConnectionHealth> GetHealthCoreAsync(IConnectionHandle handle, CancellationToken ct) { var sw = System.Diagnostics.Stopwatch.StartNew(); var isHealthy = await TestCoreAsync(handle, ct); sw.Stop(); return new ConnectionHealth(isHealthy, isHealthy ? "Slack is reachable" : "Slack is not responding", sw.Elapsed, DateTimeOffset.UtcNow); }
