...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.SaaS
{
    public class DocuSignConnectionStrategy : SaaSConnectionStrategyBase
    {
        ...
        protected override async Task<IConnectionHandle> ConnectCoreAsync(ConnectionConfig config, CancellationToken ct) { var account = GetConfiguration<string>(config, "AccountId", null!); var endpoint = $"https://{account}.docusign.net"; var httpClient = new HttpClient { BaseAddress = new Uri(endpoint), Timeout = config.Timeout }; return new DefaultConnectionHandle(httpClient, new Dictionary<string, object> { ["AccountId"] = account, ["Endpoint"] = endpoint }); }
