...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.SaaS
{
    public class DocuSignConnectionStrategy : SaaSConnectionStrategyBase
    {
        ...
        public override string SemanticDescription => "Connects to DocuSign electronic signature platform using HTTPS REST API.";
        ...
        protected override async Task<IConnectionHandle> ConnectCoreAsync(ConnectionConfig config, CancellationToken ct) { var account = GetConfiguration<string>(config, "AccountId", string.Empty); var endpoint = $"https://{account}.docusign.net"; var httpClient = new HttpClient { BaseAddress = new Uri(endpoint), Timeout = config.Timeout }; return new DefaultConnectionHandle(httpClient, new Dictionary<string, object> { ["AccountId"] = account, ["Endpoint"] = endpoint }); }
        protected override async Task<bool> TestCoreAsync(IConnectionHandle handle, CancellationToken ct) { try { var response = await handle.GetConnection<HttpClient>().GetAsync("/restapi/v2.1/accounts", ct); return response.StatusCode != System.Net.HttpStatusCode.ServiceUnavailable; } catch { return false; } }
