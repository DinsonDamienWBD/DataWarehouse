...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.SaaS
{
    public class OracleFusionConnectionStrategy : SaaSConnectionStrategyBase
    {
        ...
        protected override async Task<IConnectionHandle> ConnectCoreAsync(ConnectionConfig config, CancellationToken ct) { var pod = GetConfiguration<string>(config, "Pod", null!); var endpoint = $"https://{pod}.oraclecloud.com"; var httpClient = new HttpClient { BaseAddress = new Uri(endpoint), Timeout = config.Timeout }; return new DefaultConnectionHandle(httpClient, new Dictionary<string, object> { ["Pod"] = pod, ["Endpoint"] = endpoint }); }
