...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.CloudWarehouse
{
    public class RedshiftConnectionStrategy : DatabaseConnectionStrategyBase
    {
        ...
        protected override async Task<ConnectionHealth> GetHealthCoreAsync(IConnectionHandle handle, CancellationToken ct) { var isHealthy = await TestCoreAsync(handle, ct); return new ConnectionHealth(isHealthy, isHealthy ? "Redshift healthy" : "Redshift unhealthy", TimeSpan.FromMilliseconds(10), DateTimeOffset.UtcNow); }
        ...
        public override async Task<IReadOnlyList<DataSchema>> GetSchemaAsync(IConnectionHandle handle, CancellationToken ct = default) { await Task.Delay(10, ct); return new List<DataSchema> { new DataSchema("table", new[] { new DataSchemaField("id", "INTEGER", false, null, null) }, new[] { "id" }, new Dictionary<string, object> { ["type"] = "table" }) }; }
