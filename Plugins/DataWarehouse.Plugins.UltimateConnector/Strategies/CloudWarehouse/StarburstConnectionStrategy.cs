...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.CloudWarehouse
{
    public class StarburstConnectionStrategy : DatabaseConnectionStrategyBase
    {
        ...
        protected override async Task<bool> TestCoreAsync(IConnectionHandle handle, CancellationToken ct) { if (_httpClient == null) return false; try { var response = await _httpClient.GetAsync("/v1/info", ct); return response.IsSuccessStatusCode; } catch (OperationCanceledException) { throw; } catch { return false; } }
        ...
        protected override async Task<ConnectionHealth> GetHealthCoreAsync(IConnectionHandle handle, CancellationToken ct)
        {
            ...
            sw.Stop();
            return new ConnectionHealth(isHealthy, isHealthy ? "Starburst healthy" : "Starburst unhealthy", sw.Elapsed, DateTimeOffset.UtcNow);
        }
