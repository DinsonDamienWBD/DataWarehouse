...
/// <summary>
/// PostgreSQL connection strategy using Npgsql driver.
/// Provides production-ready connectivity to PostgreSQL 9.6+ database servers.
/// </summary>
public sealed class PostgreSqlConnectionStrategy : DatabaseConnectionStrategyBase
{
    ...
    public override string SemanticDescription =>
        "PostgreSQL relational database connection. Supports ACID transactions, complex queries, " +
        "JSON/JSONB storage, full-text search, and advanced indexing. Ideal for transactional workloads, " +
        "analytics, and applications requiring data integrity guarantees.";

    /// <inheritdoc/>
    public override string[] Tags =>
    [
        "postgresql", "postgres", "sql", "relational", "acid", "transactional",
        "json", "jsonb", "open-source", "database"
    ];

    /// <summary>
    /// Initializes a new instance of <see cref="PostgreSqlConnectionStrategy"/>.
    /// </summary>
    ...
    protected override async Task<IConnectionHandle> ConnectCoreAsync(ConnectionConfig config, CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(connectionString))
            throw new ArgumentException("Connection string is required for PostgreSQL connection.");
    ...
    protected override async Task<ConnectionHealth> GetHealthCoreAsync(IConnectionHandle handle, CancellationToken ct)
    {
        ...
        try
        {
            ...
            return new ConnectionHealth(
                IsHealthy: true,
                StatusMessage: $"PostgreSQL {connection.ServerVersion} - Database: {connection.Database}",
                Latency: sw.Elapsed,
                CheckedAt: DateTimeOffset.UtcNow);
        }
        catch (Exception ex)
        ...
        ConnectionConfig config, CancellationToken ct = default)
    {
        ...
        {
            errors.Add("ConnectionString is required for PostgreSQL connection.");
        }
        else
        {
            // Validate connection string format
            ...
            {
                errors.Add($"Invalid PostgreSQL connection string format: {ex.Message}");
            }
        }
