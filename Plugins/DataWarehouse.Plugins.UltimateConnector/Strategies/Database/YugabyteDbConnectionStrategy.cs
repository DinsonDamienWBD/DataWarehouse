...
/// <summary>
/// YugabyteDB connection strategy using Npgsql driver (PostgreSQL wire protocol).
/// Provides production-ready connectivity to YugabyteDB distributed SQL database.
/// </summary>
public sealed class YugabyteDbConnectionStrategy : DatabaseConnectionStrategyBase
{
    ...
    /// <inheritdoc/>
    public override string DisplayName => "YugabyteDB";
    ...
    public override string SemanticDescription =>
        "YugabyteDB distributed SQL database connection. Cloud-native database with PostgreSQL compatibility, " +
        "automatic sharding, replication, and geo-distribution. Provides ACID transactions, low-latency reads, " +
        "and horizontal scalability. Ideal for cloud-native applications requiring global distribution.";

    /// <inheritdoc/>
    public override string[] Tags =>
    [
        "yugabytedb", "yugabyte", "distributed", "sql", "postgresql-compatible",
        "cloud-native", "geo-distributed", "acid", "scalable", "database"
    ];

    /// <summary>
    /// Initializes a new instance of <see cref="YugabyteDbConnectionStrategy"/>.
    /// </summary>
    ...
    protected override async Task<IConnectionHandle> ConnectCoreAsync(ConnectionConfig config, CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(connectionString))
            throw new ArgumentException("Connection string is required for YugabyteDB connection.");
    ...
    protected override async Task<ConnectionHealth> GetHealthCoreAsync(IConnectionHandle handle, CancellationToken ct)
    {
        ...
        try
        {
            ...
            return new ConnectionHealth(true, $"YugabyteDB {version} - Database: {connection.Database}", sw.Elapsed, DateTimeOffset.UtcNow);
        }
        catch (Exception ex)
        {
            sw.Stop();
    ...
    public override Task<(bool IsValid, string[] Errors)> ValidateConfigAsync(ConnectionConfig config, CancellationToken ct = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(config.ConnectionString))
            errors.Add("ConnectionString is required for YugabyteDB connection.");
        else
        {
            try
            {
