...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.Messaging
{
    public class AmazonMskConnectionStrategy : MessagingConnectionStrategyBase
    {
        ...
        {
            // MSK uses Kafka wire protocol - reuse Kafka implementation
            ...
            var clientId = "datawarehouse-msk-connector";
        ...
        public override async IAsyncEnumerable<byte[]> SubscribeAsync(IConnectionHandle handle, string topic, string? consumerGroup = null, [EnumeratorCancellation] CancellationToken ct = default)
        {
            ...
            while (!ct.IsCancellationRequested && client.Connected)
            {
                ...
                var clientId = "datawarehouse-msk-connector";
                var correlationId = Environment.TickCount;
                writer.Write((short)1); // Fetch
                writer.Write((short)0);
                writer.Write(correlationId);
                WriteKafkaString(writer, clientId);
                writer.Write(-1); // Replica
                writer.Write(100); // Max wait
        ...
        private static byte[] BuildKafkaMessageSet(byte[] value)
        {
            ...
            msgWriter.Write((byte)0); msgWriter.Write((byte)0);
            msgWriter.Write(-1);
        ...
        private static uint ComputeCrc32(byte[] data) { uint crc = 0xFFFFFFFF; foreach (var b in data) crc = (crc >> 8) ^ Crc32Table[(crc ^ b) & 0xFF]; return crc ^ 0xFFFFFFFF; }
        private static readonly uint[] Crc32Table = GenerateCrc32Table();
        private static uint[] GenerateCrc32Table() { var table = new uint[256]; for (uint i = 0; i < 256; i++) { var crc = i; for (var j = 0; j < 8; j++) crc = (crc & 1) != 0 ? (crc >> 1) ^ 0xEDB88320 : crc >> 1; table[i] = crc; } return table; }
    }
}
