...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.Messaging
{
    public class ConfluentCloudConnectionStrategy : MessagingConnectionStrategyBase
    {
        ...
        protected override async Task<IConnectionHandle> ConnectCoreAsync(ConnectionConfig config, CancellationToken ct)
        {
            ...
            // Send SASL handshake request (API Key 17)
            using var ms = new System.IO.MemoryStream();
            using var writer = new System.IO.BinaryWriter(ms);
            writer.Write((short)17); // API Key: SaslHandshake
            writer.Write((short)1); // API Version
            writer.Write(1); // Correlation ID
            WriteKafkaString(writer, "datawarehouse-confluent");
            WriteKafkaString(writer, "PLAIN"); // SASL mechanism
        ...
        public override async Task PublishAsync(IConnectionHandle handle, string topic, byte[] message, Dictionary<string, string>? headers = null, CancellationToken ct = default)
        {
            ...
            if (client == null || !client.Connected)
                throw new InvalidOperationException("Confluent Cloud connection is not established");
            var stream = client.GetStream();
            // Kafka Produce Request (API Key 0) with SASL
            using var ms = new System.IO.MemoryStream();
            using var writer = new System.IO.BinaryWriter(ms);
            var clientId = "datawarehouse-confluent";
            var correlationId = Environment.TickCount;
            writer.Write((short)0); // API Key: Produce
            writer.Write((short)3); // API Version 3 (supports headers)
            writer.Write(correlationId);
            WriteKafkaString(writer, clientId);
            writer.Write((short)-1); // Required Acks
            writer.Write(5000); // Timeout ms
