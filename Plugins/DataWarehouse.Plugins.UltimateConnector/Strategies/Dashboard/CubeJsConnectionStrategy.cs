...
public sealed class CubeJsConnectionStrategy : DashboardConnectionStrategyBase
{
    ...
    protected override async Task<bool> TestCoreAsync(IConnectionHandle handle, CancellationToken ct){try { using var response = await handle.GetConnection<HttpClient>().GetAsync("/cubejs-api/v1/meta", ct); return response.IsSuccessStatusCode; } catch { return false; }}
    ...
    protected override async Task<ConnectionHealth> GetHealthCoreAsync(IConnectionHandle handle, CancellationToken ct){var sw = System.Diagnostics.Stopwatch.StartNew();try { using var response = await handle.GetConnection<HttpClient>().GetAsync("/readyz", ct); sw.Stop(); return new ConnectionHealth(response.IsSuccessStatusCode, "Cube.js ready", sw.Elapsed, DateTimeOffset.UtcNow); }catch (Exception ex) { sw.Stop(); return new ConnectionHealth(false, ex.Message, sw.Elapsed, DateTimeOffset.UtcNow); }}
    ...
    public override async Task PushDataAsync(IConnectionHandle handle, string datasetId, IReadOnlyList<Dictionary<string, object?>> data, CancellationToken ct = default){var json = JsonSerializer.Serialize(new { query = data });using var content = new StringContent(json, Encoding.UTF8, "application/json");using var response = await handle.GetConnection<HttpClient>().PostAsync("/cubejs-api/v1/load", content, ct);response.EnsureSuccessStatusCode();}
}
