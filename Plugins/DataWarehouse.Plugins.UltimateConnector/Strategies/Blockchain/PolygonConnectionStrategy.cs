...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.Blockchain
{
    public class PolygonConnectionStrategy : BlockchainConnectionStrategyBase
    {
        ...
        protected override Task<ConnectionHealth> GetHealthCoreAsync(IConnectionHandle handle, CancellationToken ct) => Task.FromResult(new ConnectionHealth(true, "Polygon node", TimeSpan.Zero, DateTimeOffset.UtcNow));
