...
public sealed class ZipkinConnectionStrategy : ObservabilityConnectionStrategyBase
{
    ...
    {
        try { var response = await handle.GetConnection<HttpClient>().GetAsync("/health", ct); return response.IsSuccessStatusCode; } catch { return false; /* Connection validation - failure acceptable */ }
    }
