    ...
    public class GcpSpannerConnectionStrategy : SaaSConnectionStrategyBase
    {
        ...
        protected override Task<(string Token, DateTimeOffset Expiry)> AuthenticateAsync(IConnectionHandle handle, CancellationToken ct = default)
            => throw new NotSupportedException(
                "GCP Spanner authentication requires OAuth2 via Google Application Default Credentials " +
                "or a service account JSON key. Set GOOGLE_APPLICATION_CREDENTIALS and use the " +
                "Google.Cloud.Spanner.Data NuGet package for production connectivity.");
