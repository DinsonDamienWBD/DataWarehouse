...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.CloudPlatform
{
    ...
    {
        public override string StrategyId => "gcp-bigtable";
        public override string DisplayName => "GCP Bigtable";
        ...
        protected override async Task<bool> TestCoreAsync(IConnectionHandle handle, CancellationToken ct) { try { var response = await handle.GetConnection<HttpClient>().GetAsync("/v2/projects", ct); return response.IsSuccessStatusCode; } catch (OperationCanceledException) { throw; } catch { return false; } }
        ...
        protected override Task<(string Token, DateTimeOffset Expiry)> AuthenticateAsync(IConnectionHandle handle, CancellationToken ct = default) { var token = handle.ConnectionInfo.TryGetValue("AuthToken", out var t) ? t?.ToString() ?? string.Empty : string.Empty; return Task.FromResult((token, DateTimeOffset.UtcNow.AddHours(1))); }
