...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.CloudPlatform
{
    public class DigitalOceanSpacesConnectionStrategy : SaaSConnectionStrategyBase
    {
        ...
        protected override async Task<bool> TestCoreAsync(IConnectionHandle handle, CancellationToken ct) { try { var response = await handle.GetConnection<HttpClient>().GetAsync("/", ct); return response.IsSuccessStatusCode || response.StatusCode == System.Net.HttpStatusCode.Forbidden; } catch (OperationCanceledException) { throw; } catch { return false; } }
