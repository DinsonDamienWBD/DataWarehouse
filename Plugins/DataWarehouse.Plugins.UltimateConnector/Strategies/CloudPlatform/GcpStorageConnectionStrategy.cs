...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.CloudPlatform
{
    public class GcpStorageConnectionStrategy : SaaSConnectionStrategyBase
    {
        ...
        public override string SemanticDescription => "Connects to Google Cloud Storage using HTTPS REST API for scalable object storage.";
        public override string[] Tags => new[] { "gcp", "storage", "cloud", "object-storage", "rest-api" };
        public GcpStorageConnectionStrategy(ILogger? logger = null) : base(logger) { }
        protected override async Task<IConnectionHandle> ConnectCoreAsync(ConnectionConfig config, CancellationToken ct) { var httpClient = new HttpClient { BaseAddress = new Uri("https://storage.googleapis.com"), Timeout = config.Timeout }; return new DefaultConnectionHandle(httpClient, new Dictionary<string, object> { ["Endpoint"] = "https://storage.googleapis.com" }); }
        protected override async Task<bool> TestCoreAsync(IConnectionHandle handle, CancellationToken ct) { try { var response = await handle.GetConnection<HttpClient>().GetAsync("/storage/v1/b", ct); return response.IsSuccessStatusCode || response.StatusCode == System.Net.HttpStatusCode.Unauthorized; } catch { return false; } }
