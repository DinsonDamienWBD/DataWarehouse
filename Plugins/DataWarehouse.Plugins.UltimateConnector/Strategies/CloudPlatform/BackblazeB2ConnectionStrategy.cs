...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.CloudPlatform
{
    public class BackblazeB2ConnectionStrategy : SaaSConnectionStrategyBase
    {
        ...
        protected override async Task<bool> TestCoreAsync(IConnectionHandle handle, CancellationToken ct) { try { var response = await handle.GetConnection<HttpClient>().GetAsync("/b2api/v2/b2_authorize_account", ct); return response.StatusCode != System.Net.HttpStatusCode.ServiceUnavailable; } catch { return false; } }
