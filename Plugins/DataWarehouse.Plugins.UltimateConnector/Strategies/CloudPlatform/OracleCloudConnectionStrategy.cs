...
namespace DataWarehouse.Plugins.UltimateConnector.Strategies.CloudPlatform
{
    public class OracleCloudConnectionStrategy : SaaSConnectionStrategyBase
    {
        ...
        protected override async Task<IConnectionHandle> ConnectCoreAsync(ConnectionConfig config, CancellationToken ct) { var region = GetConfiguration(config, "Region", "us-ashburn-1"); var namespaceName = GetConfiguration<string>(config, "Namespace", string.Empty); var endpoint = $"https://objectstorage.{region}.oraclecloud.com"; var httpClient = new HttpClient { BaseAddress = new Uri(endpoint), Timeout = config.Timeout }; return new DefaultConnectionHandle(httpClient, new Dictionary<string, object> { ["Region"] = region, ["Namespace"] = namespaceName, ["Endpoint"] = endpoint }); }
        protected override async Task<bool> TestCoreAsync(IConnectionHandle handle, CancellationToken ct) { try { var response = await handle.GetConnection<HttpClient>().GetAsync("/n/", ct); return response.IsSuccessStatusCode; } catch (OperationCanceledException) { throw; } catch { return false; } }
