...
public sealed class ThriftStrategy : DataFormatStrategyBase
{
    ...
    protected override Task InitializeAsyncCore(CancellationToken cancellationToken) { IncrementCounter("thrift.init"); return base.InitializeAsyncCore(cancellationToken); }
    /// <summary>Production hardening: graceful shutdown.</summary>
    protected override Task ShutdownAsyncCore(CancellationToken cancellationToken) { IncrementCounter("thrift.shutdown"); return base.ShutdownAsyncCore(cancellationToken); }
    /// <summary>Production hardening: cached health check.</summary>
    public Task<StrategyHealthCheckResult> CheckHealthAsync(CancellationToken ct = default) =>
        GetCachedHealthAsync(async (c) => new StrategyHealthCheckResult(true, "Apache Thrift strategy ready", new Dictionary<string, object> { ["ParseOps"] = GetCounter("thrift.parse"), ["SerializeOps"] = GetCounter("thrift.serialize") }), TimeSpan.FromSeconds(60), ct);
