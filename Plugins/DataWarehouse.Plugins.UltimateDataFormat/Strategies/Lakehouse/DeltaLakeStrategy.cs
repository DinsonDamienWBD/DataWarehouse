...
public sealed class DeltaLakeStrategy : DataFormatStrategyBase
{
    ...
    protected override Task InitializeAsyncCore(CancellationToken cancellationToken) { IncrementCounter("deltalake.init"); return base.InitializeAsyncCore(cancellationToken); }
    /// <summary>Production hardening: graceful shutdown.</summary>
    protected override Task ShutdownAsyncCore(CancellationToken cancellationToken) { IncrementCounter("deltalake.shutdown"); return base.ShutdownAsyncCore(cancellationToken); }
    /// <summary>Production hardening: cached health check.</summary>
    public Task<StrategyHealthCheckResult> CheckHealthAsync(CancellationToken ct = default) =>
        GetCachedHealthAsync(async (c) => new StrategyHealthCheckResult(true, "Delta Lake strategy ready", new Dictionary<string, object> { ["ParseOps"] = GetCounter("deltalake.parse"), ["SerializeOps"] = GetCounter("deltalake.serialize") }), TimeSpan.FromSeconds(60), ct);
