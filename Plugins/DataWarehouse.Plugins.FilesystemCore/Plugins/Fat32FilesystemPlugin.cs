// <copyright file="Fat32FilesystemPlugin.cs" company="DataWarehouse">
...
/// <summary>
/// Filesystem plugin for FAT32 (File Allocation Table, 32-bit).
/// </summary>
/// <remarks>
/// FAT32 is a legacy filesystem with wide compatibility but limited features:
/// <list type="bullet">
/// <item>Maximum file size: 4GB - 1 byte (4,294,967,295 bytes)</item>
/// <item>Maximum volume size: 2TB (with 512-byte sectors)</item>
/// <item>8.3 short filename support (LFN extension for long names)</item>
/// <item>No file permissions or security (always returns 777)</item>
/// <item>No journaling</item>
/// <item>No compression or encryption</item>
/// <item>No hard links or symbolic links</item>
/// <item>Volume labels up to 11 characters</item>
/// </list>
/// FAT32 is commonly used for USB drives, SD cards, and cross-platform compatibility.
/// </remarks>
[SupportedOSPlatform("windows")]
[SupportedOSPlatform("linux")]
[SupportedOSPlatform("macos")]
public sealed class Fat32FilesystemPlugin : FilesystemPluginBase
{
    /// <summary>
    /// Maximum file size supported by FAT32 (4GB - 1 byte).
    /// </summary>
    public const long MaxFileSize = 4294967295L;

    /// <summary>
    /// Maximum volume label length in characters.
    /// </summary>
    public const int MaxVolumeLabelLength = 11;

    /// <inheritdoc/>
    public override string Id => "com.datawarehouse.filesystem.fat32";

    /// <inheritdoc/>
    public override string Name => "FAT32 Filesystem";

    /// <inheritdoc/>
    public override string Version => "1.0.0";

    /// <inheritdoc/>
    public override string FilesystemType => "FAT32";

    /// <inheritdoc/>
    public override int DetectionPriority => 50; // Low priority, common fallback
    ...
    // Note: Deliberately minimal capabilities due to FAT32 limitations
    ...
    public override async Task<FilesystemStats> GetStatsAsync(string path, CancellationToken ct = default)
    {
        var stats = new FilesystemStats
        {
            ...
            MaxFileNameLength = 255, // With LFN support
            MaxPathLength = 260 // Traditional Windows limit, often applies
        };
        ...
        // FAT32-specific limitations
        stats.ExtendedStats["maxFileSize"] = MaxFileSize;
        stats.ExtendedStats["shortNameSupport"] = true;
        stats.ExtendedStats["longFileNameSupport"] = true;
    ...
    {
        var fsType = await GetFilesystemTypeAsync(path, ct);
        if (!IsFat32Type(fsType))
            return FilesystemCapabilities.None;

        var capabilities = FilesystemCapabilities.None;

        // FAT32 always supports these (minimal set)
        capabilities |= FilesystemCapabilities.Unicode; // Via LFN
        ...
        // - CaseSensitive (case-preserving but case-insensitive)
        // - LargeFiles (>4GB)
        // - Reflinks
    ...
    public override async Task<FilesystemHealth> CheckHealthAsync(string path, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // Warn about FAT32 limitations
            health.Issues.Add(new FilesystemIssue
            {
                Severity = FilesystemIssueSeverity.Info,
                Code = "FAT32_LIMITATIONS",
                Message = "FAT32 has a 4GB maximum file size limit",
                Remediation = "Consider using exFAT or NTFS for large files"
            });
        }
        catch (Exception ex)
        {
    ...
    {
        var handle = new Fat32FilesystemHandle(path, options, Capabilities);
        return Task.FromResult<IFilesystemHandle>(handle);
    }

    #region Helper Methods

    private static bool IsFat32Type(string fsType)
    {
        return string.Equals(fsType, "FAT32", StringComparison.OrdinalIgnoreCase) ||
    ...
    private static async Task<string> GetFilesystemTypeAsync(string path, CancellationToken ct)
    {
        if (IsWindows)
        {
            return await GetWindowsFilesystemTypeAsync(path);
        }
        else
        {
    ...
    private static Task<string> GetWindowsFilesystemTypeAsync(string path)
    {
        try
        {
            ...
            return Task.FromResult(driveInfo.DriveFormat);
        }
        catch
        {
            return Task.FromResult(string.Empty);
    ...
    private static Task GetWindowsStatsAsync(string path, FilesystemStats stats)
    {
        try
        {
            ...
            // FAT32 cluster sizes vary, typically 4KB-32KB
...
/// <summary>
/// Filesystem handle implementation for FAT32.
/// </summary>
internal sealed class Fat32FilesystemHandle : IFilesystemHandle
{
    private readonly MountOptions _options;
    private volatile bool _isValid = true;

    /// <summary>
    /// Initializes a new instance of the <see cref="Fat32FilesystemHandle"/> class.
    /// </summary>
    ...
    public Fat32FilesystemHandle(string mountPath, MountOptions options, FilesystemCapabilities capabilities)
    {
        MountPath = mountPath;
    ...
    public Task<Stream> WriteFileAsync(string path, CancellationToken ct = default)
    {
        ...
        // Check current file size - FAT32 cannot grow files beyond 4GB
        if (File.Exists(fullPath))
        {
            var info = new FileInfo(fullPath);
            if (info.Length >= Fat32FilesystemPlugin.MaxFileSize)
            {
                throw new IOException($"File size ({info.Length} bytes) is at FAT32 maximum limit.");
            }
        }
    ...
    public Task<Stream> AppendFileAsync(string path, CancellationToken ct = default)
    {
        ...
        // Check current file size for FAT32 limit
        if (File.Exists(fullPath))
        {
            var info = new FileInfo(fullPath);
            if (info.Length >= Fat32FilesystemPlugin.MaxFileSize)
    ...
    public async Task CopyFileAsync(string sourcePath, string destinationPath, bool overwrite = false, CancellationToken ct = default)
    {
        ...
        // Check source file size for FAT32 limit (in case destination is FAT32)
        var sourceInfo = new FileInfo(sourceFullPath);
        if (sourceInfo.Length > Fat32FilesystemPlugin.MaxFileSize)
        {
            throw new IOException($"Source file size ({sourceInfo.Length} bytes) exceeds FAT32 maximum of {Fat32FilesystemPlugin.MaxFileSize} bytes.");
        }
        ...
        [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        foreach (var entry in entries)
        {
            ...
            yield return new FilesystemEntry
            {
                ...
                IsSymbolicLink = false, // FAT32 doesn't support symlinks
    ...
    public Task<FilesystemAttributes> GetAttributesAsync(string path, CancellationToken ct = default)
    {
        ...
        return Task.FromResult(new FilesystemAttributes
        {
            ...
            IsSymbolicLink = false, // FAT32 doesn't support symlinks
            LinkCount = 1, // FAT32 doesn't support hard links
            // FAT32 has no permissions - effectively 777
            UnixPermissions = 0x1FF // 0777 in octal
        });
    }
    ...
    public Task SetAttributesAsync(string path, FilesystemAttributes attributes, CancellationToken ct = default)
    {
        ...
        // FAT32 only supports basic file attributes (not permissions)
    ...
    {
        // FAT32 doesn't support extended attributes
    ...
    {
        // FAT32 doesn't support extended attributes - silently ignore
        return Task.CompletedTask;
    }
    ...
    public Task<IFileLock> LockFileAsync(string path, TimeSpan? timeout = null, CancellationToken ct = default)
    {
        ThrowIfInvalid();
        var fullPath = GetFullPath(path);
        return Task.FromResult<IFileLock>(new Fat32FileLock(fullPath, isExclusive: true));
    }
    ...
    public Task<IFileLock> LockFileSharedAsync(string path, TimeSpan? timeout = null, CancellationToken ct = default)
    {
        ThrowIfInvalid();
        var fullPath = GetFullPath(path);
        return Task.FromResult<IFileLock>(new Fat32FileLock(fullPath, isExclusive: false));
    }
    ...
    {
        throw new NotSupportedException("FAT32 does not support hard links.");
    }

    /// <inheritdoc/>
    public Task CreateSymbolicLinkAsync(string targetPath, string linkPath, CancellationToken ct = default)
    {
        throw new NotSupportedException("FAT32 does not support symbolic links.");
    }
    ...
    {
        if (!_isValid)
            throw new ObjectDisposedException(nameof(Fat32FilesystemHandle));
    }
...
/// <summary>
/// FAT32 file lock implementation.
/// </summary>
internal sealed class Fat32FileLock : IFileLock
{
    private FileStream? _stream;

    /// <summary>
    /// Initializes a new instance of the <see cref="Fat32FileLock"/> class.
    /// </summary>
    /// <param name="path">The file path to lock.</param>
    /// <param name="isExclusive">Whether to acquire an exclusive lock.</param>
    public Fat32FileLock(string path, bool isExclusive)
    {
        Path = path;
