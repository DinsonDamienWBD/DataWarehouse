// <copyright file="Nilfs2FilesystemPlugin.cs" company="DataWarehouse">
...
/// <summary>
/// Filesystem plugin for NILFS2 (New Implementation of a Log-structured File System) - Linux.
/// </summary>
/// <remarks>
/// NILFS2 capabilities include:
/// <list type="bullet">
/// <item>Hard links and symbolic links</item>
/// <item>Extended attributes (xattr)</item>
/// <item>POSIX ACLs</item>
/// <item>Continuous snapshots (checkpoints)</item>
/// <item>Log-structured design</item>
/// <item>Journaling via log structure</item>
/// <item>Fast recovery after system crash</item>
/// <item>Snapshot management (lscp, mkcp, rmcp commands)</item>
/// <item>Sparse files</item>
/// </list>
/// NILFS2 automatically creates snapshots at regular intervals.
/// </remarks>
[SupportedOSPlatform("linux")]
public sealed partial class Nilfs2FilesystemPlugin : FilesystemPluginBase
{
    /// <inheritdoc/>
    public override string Id => "com.datawarehouse.filesystem.nilfs2";

    /// <inheritdoc/>
    public override string Name => "NILFS2 Filesystem";

    /// <inheritdoc/>
    public override string Version => "1.0.0";

    /// <inheritdoc/>
    public override string FilesystemType => "nilfs2";

    /// <inheritdoc/>
    public override int DetectionPriority => 190; // Standard priority
    ...
    public override FilesystemCapabilities Capabilities =>
        ...
        FilesystemCapabilities.Snapshots | // Continuous checkpoints
        FilesystemCapabilities.Journaling | // Log-structured
    ...
    public override async Task<bool> CanHandleAsync(string path, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            return string.Equals(fsType, "nilfs2", StringComparison.OrdinalIgnoreCase);
        }
        catch
        {
            return false;
    ...
    public override async Task<FilesystemStats> GetStatsAsync(string path, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // Get NILFS2-specific information using lscp (list checkpoints)
            try
            {
                var lscpOutput = await RunCommandAsync("lscp", $"\"{normalizedPath}\"", ct);
                var checkpointCount = lscpOutput.Split('\n', StringSplitOptions.RemoveEmptyEntries).Length - 1; // Exclude header
                stats.ExtendedStats["checkpoint_count"] = checkpointCount.ToString();

                // Get latest checkpoint
                var lines2 = lscpOutput.Split('\n', StringSplitOptions.RemoveEmptyEntries);
                if (lines2.Length > 1)
                {
                    var latestLine = lines2[^1]; // Last line
                    var cpMatch = CheckpointRegex().Match(latestLine);
                    if (cpMatch.Success)
                    {
                        stats.ExtendedStats["latest_checkpoint"] = cpMatch.Groups[1].Value;
                    }
                }
            }
            catch
            {
                // lscp not available or no permissions
                stats.ExtendedStats["checkpoint_count"] = "unavailable";
            }

            // Try to get nilfs-tune info (requires root)
            var device = stats.DevicePath;
            if (!string.IsNullOrEmpty(device))
            {
                try
                {
                    var tuneOutput = await RunCommandAsync("nilfs-tune", $"-l \"{device}\"", ct);

                    // Parse UUID
                    var uuidMatch = UuidRegex().Match(tuneOutput);
                    if (uuidMatch.Success && Guid.TryParse(uuidMatch.Groups[1].Value, out var uuid))
                        stats.FilesystemId = uuid;

                    // Parse block size
                    var blockMatch = BlockSizeRegex().Match(tuneOutput);
                    if (blockMatch.Success && int.TryParse(blockMatch.Groups[1].Value, out var blockSize))
                        stats.BlockSize = blockSize;
                    else
                        stats.BlockSize = 4096; // Default NILFS2 block size

                    // Parse checkpoint interval
                    var intervalMatch = IntervalRegex().Match(tuneOutput);
                    if (intervalMatch.Success)
                        stats.ExtendedStats["checkpoint_interval"] = intervalMatch.Groups[1].Value + "s";
                }
                catch
                {
                    stats.BlockSize = 4096; // Default NILFS2 block size
    ...
    public override async Task<FilesystemCapabilities> DetectCapabilitiesAsync(string path, CancellationToken ct = default)
    {
        ...
        var fsType = await GetFilesystemTypeAsync(path, ct);
        if (!string.Equals(fsType, "nilfs2", StringComparison.OrdinalIgnoreCase))
            return FilesystemCapabilities.None;

        // Return standard NILFS2 capabilities
        return Capabilities;
    }
        ...
        try
        {
            ...
            if (stats.UsagePercent > 95)
            {
                ...
                health.Issues.Add(new FilesystemIssue
                {
                    ...
                    Remediation = "Free up disk space, remove old checkpoints with rmcp, or expand the filesystem"
                });
            }
            else if (stats.UsagePercent > 90)
            {
                ...
                health.Issues.Add(new FilesystemIssue
                {
                    ...
                    Remediation = "Consider removing old checkpoints with rmcp"
                });
            }
            ...
            if (stats.InodeUsagePercent > 95)
            {
                ...
                health.Issues.Add(new FilesystemIssue
                {
                    ...
                    Remediation = "Remove files or old checkpoints"
                });
            }
            ...
            if (stats.ExtendedStats.TryGetValue("checkpoint_count", out var cpCountStr) &&
                int.TryParse(cpCountStr as string, out var cpCount) && cpCount > 10000)
            {
                health.Status = FilesystemHealthStatus.Warning;
                health.Issues.Add(new FilesystemIssue
                {
                    Severity = FilesystemIssueSeverity.Warning,
                    Code = "CHECKPOINT_EXCESSIVE",
                    Message = $"Large number of checkpoints: {cpCount}",
                    Remediation = "Remove old checkpoints using 'rmcp' to free up space"
                });
            }
            ...
            if (stats.IsReadOnly)
            {
                ...
                health.Issues.Add(new FilesystemIssue
                {
                    ...
                    Remediation = "Check dmesg for errors and run fsck.nilfs2"
                });
            }
        }
        catch (Exception ex)
    ...
    {
        if (!IsLinux)
            throw new PlatformNotSupportedException("NILFS2 plugin only works on Linux.");

        var handle = new Nilfs2FilesystemHandle(path, options, Capabilities);
    ...
    [GeneratedRegex(@"Filesystem UUID:\s+([a-f0-9-]+)", RegexOptions.IgnoreCase)]
    private static partial Regex UuidRegex();

    [GeneratedRegex(@"Block size:\s+(\d+)")]
    private static partial Regex BlockSizeRegex();

    [GeneratedRegex(@"Checkpoint interval:\s+(\d+)")]
...
/// <summary>
/// Filesystem handle implementation for NILFS2.
/// </summary>
[SupportedOSPlatform("linux")]
internal sealed class Nilfs2FilesystemHandle : IFilesystemHandle
{
    private readonly MountOptions _options;
    private volatile bool _isValid = true;

    public Nilfs2FilesystemHandle(string mountPath, MountOptions options, FilesystemCapabilities capabilities)
    {
        MountPath = mountPath;
    ...
    public Task<IFileLock> LockFileAsync(string path, TimeSpan? timeout = null, CancellationToken ct = default)
    {
        ThrowIfInvalid();
        var fullPath = GetFullPath(path);
        return Task.FromResult<IFileLock>(new Nilfs2FileLock(fullPath, isExclusive: true));
    }
    ...
    public Task<IFileLock> LockFileSharedAsync(string path, TimeSpan? timeout = null, CancellationToken ct = default)
    {
        ThrowIfInvalid();
        var fullPath = GetFullPath(path);
        return Task.FromResult<IFileLock>(new Nilfs2FileLock(fullPath, isExclusive: false));
    }
    ...
    {
        if (!_isValid)
            throw new ObjectDisposedException(nameof(Nilfs2FilesystemHandle));
    }
...
/// <summary>
/// NILFS2 file lock implementation using flock.
/// </summary>
[SupportedOSPlatform("linux")]
internal sealed class Nilfs2FileLock : IFileLock
{
    private FileStream? _stream;

    public Nilfs2FileLock(string path, bool isExclusive)
    {
        Path = path;
