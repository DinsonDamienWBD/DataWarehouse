...
public sealed class AirGapBridgePlugin : InfrastructurePluginBase, IDisposable
{
    ...
    public override async Task StartAsync(CancellationToken ct)
    {
        ...
        // Persist or restore master key so exported packages remain readable after restart
        var savedKey = await LoadStateAsync("masterKey", ct);
        if (savedKey != null && savedKey.Length == 32)
        {
            // Wipe the randomly-generated constructor key and use the persisted one
            CryptographicOperations.ZeroMemory(_masterKey);
            _masterKey = savedKey;

            // Re-initialize components that depend on _masterKey
            InitializeComponents();
        }
        else
        {
            // First run: persist the key generated by the constructor
            await SaveStateAsync("masterKey", _masterKey, ct);
        }
