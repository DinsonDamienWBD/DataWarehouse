...
namespace DataWarehouse.Plugins.UltimateDataLake.Delegation;

/// <summary>
/// Wraps message bus delegation calls with circuit breaker protection and local cache fallback.
/// When the authoritative plugin (e.g. UltimateDataLineage or UltimateDataCatalog) is unavailable or slow,
...
internal sealed class MessageBusDelegationHelper : IDisposable
{
    ...
    /// <param name="targetTopic">The topic prefix for delegation (e.g. "lineage" or "catalog").</param>
    ...
    /// </summary>
    /// <param name="topic">The full topic to send to (e.g. "lineage.track", "catalog.register").</param>
    ...
    private static string BuildCacheKey(string topic, Dictionary<string, object> payload)
    {
        ...
        var dataObjectId = payload.TryGetValue("dataObjectId", out var doObj) && doObj is string doStr ? doStr : "";

        return $"{topic}|{action}|{id}|{nodeId}|{sourceNodeId}|{targetNodeId}|{query}|{dataObjectId}";
    }
