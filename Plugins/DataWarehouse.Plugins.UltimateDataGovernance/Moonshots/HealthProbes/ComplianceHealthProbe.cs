...
/// <summary>
/// Health probe for the Compliance Passports moonshot (MoonshotId.CompliancePassports).
/// Checks plugin responsiveness, loaded regulations, configuration validity,
/// and bus connectivity for compliance passport operations.
/// </summary>
public sealed class ComplianceHealthProbe : IMoonshotHealthProbe
{
    ...
    public MoonshotId MoonshotId => MoonshotId.CompliancePassports;
    public TimeSpan HealthCheckInterval => TimeSpan.FromSeconds(60);

    public ComplianceHealthProbe(
        IMessageBus messageBus,
        MoonshotConfiguration config,
        ILogger<ComplianceHealthProbe> logger)
    {
    ...
    public async Task<MoonshotHealthReport> CheckHealthAsync(CancellationToken ct)
    {
        ...
        var featureConfig = _config.GetEffectiveConfig(MoonshotId.CompliancePassports);
        components["Config"] = featureConfig.Enabled
            ? new MoonshotComponentHealth("Config", MoonshotReadiness.Ready, "Compliance Passports enabled")
            : new MoonshotComponentHealth("Config", MoonshotReadiness.NotReady, "Compliance Passports disabled in configuration");

        components["Plugin"] = await CheckBusTopicAsync("compliance.passport.health.ping", "Plugin", ct);

        components["Strategy"] = await CheckRegulationsAsync(ct);

        components["Bus"] = await CheckBusTopicAsync("compliance.echo", "Bus", ct);

        sw.Stop();
        var overall = ComputeOverallReadiness(components);

        _logger.LogDebug("Compliance health check completed in {Duration}ms: {Readiness}",
            sw.ElapsedMilliseconds, overall);

        return new MoonshotHealthReport(
            MoonshotId.CompliancePassports,
            overall,
            $"Compliance Passports: {overall}",
            components,
            DateTimeOffset.UtcNow,
            sw.Elapsed);
    }

    private async Task<MoonshotComponentHealth> CheckRegulationsAsync(CancellationToken ct)
    {
        try
        {
            ...
            if (!response.Success)
                return new MoonshotComponentHealth("Strategy", MoonshotReadiness.NotReady,
                    response.ErrorMessage ?? "Regulation list request failed");

            if (response.Payload is ICollection<object> regulations && regulations.Count > 0)
                return new MoonshotComponentHealth("Strategy", MoonshotReadiness.Ready,
                    $"{regulations.Count} regulation(s) loaded");
        ...
        {
            return new MoonshotComponentHealth("Strategy", MoonshotReadiness.NotReady,
                "Regulation list request timed out");
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Compliance regulation check failed");
        ...
        string topic, string componentName, CancellationToken ct)
    {
        ...
        {
            _logger.LogWarning(ex, "Compliance health probe failed for topic {Topic}", topic);
