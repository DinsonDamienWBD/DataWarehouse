...
public sealed class UltimateDataGovernancePlugin : IntelligenceAwareDataManagementPluginBase, IDisposable
{
    ...
    private Task HandlePolicyAsync(PluginMessage message)
    {
        ...
        switch (action.ToLowerInvariant())
        {
            ...
            case "get":
                ...
                message.Payload["policy"] = _policies.TryGetValue(id, out var p) ? (object)p : null!;
                break;
            case "list":
    ...
    private Task HandleOwnershipAsync(PluginMessage message)
    {
        ...
        switch (action.ToLowerInvariant())
        {
            ...
            case "get":
                ...
                message.Payload["ownership"] = _ownerships.TryGetValue(assetId, out var o) ? (object)o : null!;
                break;
            case "list":
    ...
    private Task HandleClassifyAsync(PluginMessage message)
    {
        ...
        switch (action.ToLowerInvariant())
        {
            ...
            case "get":
                var assetId = GetRequiredString(message.Payload, "assetId");
                message.Payload["classification"] = _classifications.TryGetValue(assetId, out var c) ? (object)c : null!;
                break;
            case "list":
